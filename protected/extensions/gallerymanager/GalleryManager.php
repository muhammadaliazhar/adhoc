...
class GalleryManager extends CWidget {
    ...
    public function run(){
        ...
        $currentListingId = explode("/listings2/",Yii::app()->request->url);
        $currentListing = Listings2::model()->findByPk($currentListingId);
        /*if (isset($currentListing) && isset($currentListing->c_category__c)) {
                $categoryOfCurrentListing = $currentListing->c_category__c;
                $criteria->condition = "libraryId=3 AND name LIKE '%" . $categoryOfCurrentListing. "%' AND mimetype LIKE '%image%'";
                $categoryPhotos = Media::model()->findAll($criteria);
