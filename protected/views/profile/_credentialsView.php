...
<div class="credentials-view" id="credentials-<?php echo $data->id?>">
	...
	<span class="credentials-name" style="font-weight:bold;">
		<?php 
		$nameSuffix = '';
		//This errors out in cases where the otherUser doesn't have a profile connected to their user
		/*if($data->userId != $webUser->id){
			$otherUser = User::model()->findByPk($data->userId);
	file_put_contents('/tmp/bugs.txt',json_encode($data->userId).json_encode($webUser->id));
			if($otherUser)
