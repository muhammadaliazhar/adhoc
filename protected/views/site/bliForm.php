...
Yii::app()->clientScript->registerScript("bliFormJS", "
    ...
    $('#sign-btn').on('click', function(evt) {
         evt.preventDefault();
        $('#sign-btn').hide();
        //changing this to an ajax call
        $.ajax({
            url: 'https://staging.tworld.x2developer.com/index.php/site/finishBli'+queryParams+'&envelopeId=$envelope->id',
            type: 'GET',
            success: function (response) {
                window.location.replace('https://staging.tworld.x2developer.com/index.php/site/RenderBLIcompletepage');
            },
            error: function(xhr) {
                $('#sign-btn').show();
                //console.log(xhr);
    ...
    $('#contact-btn').on('click', function(evt) {
        evt.preventDefault();
        window.location = 'mailto:" . $employee->c_email__c . "?subject=\"Request For More Info Regarding $listing->c_listing_number__c\"';
    })
");
            ...
            <form class="w-100">
                ...
                <div class="row w-100 d-flex my-4 justify-content-between">
                    <div class="col-auto">
                        ...
                        <button id="contact-btn" class="btn btn-sm btn-primary">Contact Agent For More Info</button>
                    </div>
                </div>
            </form>
        </div>
