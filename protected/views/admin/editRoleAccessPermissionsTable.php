...
?>
            ...
            <th><?php
                ...
                echo X2Html::hint2(
                        Yii::t('app', 'Permission required to create records in the given module.'), array(
                    'class' => 'table-hint',
                ));
                ?>
            </th>
            <th style='width:150px;'><?php
                ...
                echo X2Html::hint2(
                        Yii::t('app', 'Permission required to update records in the given module.'), array(
                    'class' => 'table-hint',
                ));
                ?>
            </th>
            <th style='width:150px;'><?php
                ...
                echo X2Html::hint2(
                        Yii::t('app', 'Permission required to delete records in the given module.'), array(
                    'class' => 'table-hint',
                ));
                ?>
            </th>
...
// presence of corresponding auth items.
                ...
                foreach ($accessGroups as $module => $accessGroup) {
                    ...
                    foreach (RoleAccessActionBase::$permissionNames as $permissionName => $keyword) {
                        if (in_array($keyword . 'Access', $accessGroup) || in_array('PrivateOwned' . $keyword . 'Access', $accessGroup)) {
                            $authItem = $auth->getAuthItem(ucfirst($module) . $keyword . 'Access');
                            $index = array_search($keyword . 'Access', $accessGroup);
                            $privateIndex = false;
                            $privateIndexOwned = false;
                            if (in_array("Private{$keyword}Access", $accessGroup)) {
                                $privateIndex = array_search("Private{$keyword}Access", $accessGroup);
                            } 
                            if (in_array("PrivateOwned{$keyword}Access", $accessGroup)) {
                                $privateIndexOwned = array_search("PrivateOwned{$keyword}Access", $accessGroup);
                            }
                            $ret.="<td>" . CHtml::checkBox(
                                            $module . "-" . $permissionName, ($auth->hasItemChild($name, $index) ||
                                            $auth->hasItemChild($name, $privateIndex) || $auth->hasItemChild($name, $privateIndexOwned)), array(
                                        'style' => 'position:relative;top:2px;',
                                        'class' => $module . "-checkbox $permissionName-checkbox"
                            ));

                            if (in_array($keyword, array('ReadOnly', 'Update', 'Full')) && ($privateIndex || $privateIndexOwned)) {
                                if ($auth->hasItemChild($name, $privateIndexOwned)) {
                                    $classBool = 'private_owned';
                                } else if ($auth->hasItemChild($name, $privateIndex)){
                                    $classBool = 'private';
                                } else {
                                    $classBool = 'all';
                                }
                                $ret.="&nbsp;" . CHtml::dropDownList(
                                                $module . "-$permissionName-privacy", $classBool, array(
                                            'all' => Yii::t('admin', "All Records"),
                                            'private' => Yii::t('admin', "Only Assigned"),
                                            'private_owned' => Yii::t('admin', "Owned"),
                                                ), array(
