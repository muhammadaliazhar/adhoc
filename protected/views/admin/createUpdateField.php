    ...
    <div style="width:600px">
        ...
        <div class="row">
            ...
            $fieldTypes = Fields::getFieldTypes('title');
             ...
             //to deal with edits of link Fields
            if($model->linkFieldType == 'linkField'){
                $model->type = "linkField";
            }
            ...
            <div class="row">
                ...
                if($model->type == "linkField"){
                      //this is for having a field that has the value of another field AKA link fields
                        
                        $fields = Fields::model()
                            ->findAllByAttributes(array('modelName' => $model->modelName, 'type' => 'link'));
                        $option = array();
                        foreach($fields as $field){
                            $options[$field->fieldName] = $field->attributeLabel;
                        }
                        echo '<div class="row">';
                        ...
                        echo $form->error($model, 'linkField'); 
                        echo '</div>';
                        
                        if(isset($model->linkField)){
                            //if the user has picked his linked field now I give them the options of fields from that record type
                            
                            $PickedFieldRecordType =  Fields::model()
                                ->findByAttributes(array('fieldName' => $model->linkField))->linkType;
                            
                            $RecordFields =  Fields::model()
                                ->findAllByAttributes(array('modelName' => $PickedFieldRecordType));
                            
                            $FieldOptions = array();
                            foreach($RecordFields as $field){
                                $FieldOptions[$field->fieldName] = $field->attributeLabel;
                            }
                ...
                if($model->type != 'timerSum'  && $model->type != "linkField") {
        ...
        <?php if($model->type != 'timerSum') { ?>
            ...
            <div class="row">
                <?php echo $form->checkBox($model, 'isBLI', array('id' => 'isBLI')); ?>
                <?php echo $form->labelEx($model, 'isBLI', array('style' => 'display:inline;')); ?>
                <?php echo $form->error($model, 'isBLI'); ?>
            </div>

            <div class="row">
                <?php echo $form->checkBox($model, 'isADV', array('id' => 'isADV')); ?>
                <?php echo $form->labelEx($model, 'isADV', array('style' => 'display:inline;')); ?>
                <?php echo $form->error($model, 'isADV'); ?>
            </div>

            <div class="row">
                <?php echo $form->checkBox($model, 'isBM', array('id' => 'isBM')); ?>
                <?php echo $form->labelEx($model, 'isBM', array('style' => 'display:inline;')); ?>
                <?php echo $form->error($model, 'isBM'); ?>
            </div>
        <?php } ?>
