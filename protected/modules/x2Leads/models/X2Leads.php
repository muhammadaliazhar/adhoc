...
class X2Leads extends X2Model {
	...
	if (isset($this->c_phone)) {
	    $this->c_phone = preg_replace('/[^0-9]/', '', $this->c_phone);
	    if (strlen($this->c_phone) <= 15 && strlen($this->c_phone) >= 10) {
		$lastFour = substr($this->c_phone, -4);
		$middleThree = substr($this->c_phone, -7,-4);
		$areaCode = substr($this->c_phone,-10,-7);
		$countryCode = substr($this->c_phone,0,-10);
		$this->c_phone = $countryCode . " (" . $areaCode . ") " . $middleThree . "-" . $lastFour;
	    }
	}
