...
function initMap() {
  ...
  autocomplete.addListener('place_changed', function() {
    ...
    $('#X2Leads_c_ttl_address__c').val('');
    $('#X2Leads_c_ttl_city__c').val('');
    $('#X2Leads_c_ttl_country__c').val('');
    $('#X2Leads_c_ttl_state_province__c').val('');
    $('#X2Leads_c_ttl_postal_zip_code__c').val('');
    ...
    place.address_components.forEach(function(addrComp){
        switch(addrComp.types[0]) {
            ...
            case 'route':
                 streetAddr += ' ' + addrComp.long_name;
                 $('#X2Leads_c_ttl_address__c').val(streetAddr);
                 if($('#location-withdrawn-checkbox').is(':checked')){
                     $('#X2Leads_c_ttl_address__c').val('Withdrawn');
                 }
                 break;
            case 'administrative_area_level_2':
                 $('#X2Leads_c_ttl_city__c').val(addrComp.long_name);
                 if($('#location-withdrawn-checkbox').is(':checked')){
                     $('#X2Leads_c_ttl_city__c').val('Withdrawn');
                 }
                 break;
            case 'country':
                 $('#X2Leads_c_ttl_country__c').val(addrComp.long_name);
                 break;
            case 'postal_code':
                 $('#X2Leads_c_ttl_postal_zip_code__c').val(addrComp.long_name);
                 if($('#location-withdrawn-checkbox').is(':checked')){
                     $('#X2Leads_c_ttl_postal_zip_code__c').val('Withdrawn');
                 }
                 break;
            case 'administrative_area_level_1':
                 $('#X2Leads_c_ttl_state_province__c').val(addrComp.long_name);
                 break;
            case 'locality':
                 $('#X2Leads_c_ttl_city__c').val(addrComp.long_name);
                 if($('#location-withdrawn-checkbox').is(':checked')){
...
Yii::app()->clientScript->registerScript('editGeoLocation', "
    ...
    $('#X2Leads_c_ttl_address__c_field a.edit-icon').click(function () {
        $('#X2Leads_c_ttl_address__c').prop('readonly', true);
        $('#X2Leads_c_ttl_city__c_field a.edit-icon').trigger('click');
        $('#X2Leads_c_ttl_city__c').prop('readonly', true);
        $('#X2Leads_c_ttl_state_province__c_field a.edit-icon').trigger('click');
        $('#X2Leads_c_ttl_state_province__c').prop('readonly', true);
        $('#X2Leads_c_ttl_postal_zip_code__c_field a.edit-icon').trigger('click');
        $('#X2Leads_c_ttl_postal_zip_code__c').prop('readonly', true);
        $('#X2Leads_c_ttl_country__c_field a.edit-icon').trigger('click');
        $('#X2Leads_c_ttl_country__c').prop('readonly', true);
        $('div#googlemap').show();
        $('div#pac-card').show();
    ...
    $('#X2Leads_c_ttl_country__c_field a.cancel-icon').click(function () {
        $('div#googlemap').hide();
        $('div#pac-card').hide();
        $('#location-withdrawn_field').hide();
    });

    $('#X2Leads_c_ttl_country__c_field a.confirm-icon').click(function () {
        $('div#googlemap').hide();
        $('div#pac-card').hide();
        $('#location-withdrawn_field').hide();
    });
...
Yii::app()->clientScript->registerScript('placeMap', "
  ...
  $('.formSection:nth-child(4)').append($('#googlemap'));
  $('#X2Leads_c_ttl_address__c_field').append($('#location-withdrawn_field'));
  $('#googlemap').hide();
  pacCard.hide();
  $('#location-withdrawn_field').hide();
  $('#X2Leads_c_ttl_city__c_field a.edit-icon').hide();
  $('#X2Leads_c_ttl_state_province__c_field a.edit-icon').hide();
  $('#X2Leads_c_ttl_postal_zip_code__c_field a.edit-icon').hide();
  $('#X2Leads_c_ttl_country__c_field a.edit-icon').hide();
", CClientScript::POS_END);
...
<div class="page-title-fixed-outer">
    <div class="page-title-fixed-inner">
        ...
        <div class="page-title icon x2Leads">
            <h2><span class="no-bold"><?php echo Yii::t('x2Leads', 'Contacts:'); ?> </span><?php echo CHtml::encode($model->name); ?></h2>
            <?php
