...
?>
<div class="page-title listings2"><h2><?php echo Yii::t('module','Create New {X}',array('{X}'=>Modules::itemDisplayName())); ?></h2></div>
...
Yii::app()->clientScript->registerCss('imageDialogCss', "
...
Yii::app()->clientScript->registerScript('selectListingImage', "
    var getImagePreview = function (keywords) {
        $.ajax({
            type: 'GET',
            url: '" . Yii::app()->baseUrl . "/index.php/media/getMediaPreviewLinks' ,
            data: {
               params: keywords
            },
            success: function(data) {
                var data = JSON.parse(data);
                $('#listing-image-dialog').empty();
                if (data.length === 0) {
                    $('#listing-image-dialog').text('No images found for selected industry');
                    return 0;
                }
                data.forEach(function(imgElem){
                    imgElem = $(imgElem).width('75px');
                    imgElem.height('75px');
                    imgElem.addClass('listing-img');
                    console.log(imgElem);
                    imgElem.on('click', function() {
                        console.log(imgElem.prop('src'));
                        console.log($('#Listings2_c_website_listing_image__c'));
                        $('#Listings2_c_website_listing_image__c').val(imgElem.prop('src'));
                        $('#Listings2_c_listing_image__c').val(imgElem.prop('src'));
                        x2.listingImageDialog.dialog('close');
                        GetridButtons();
                    });
                    $('#listing-image-dialog').prepend(imgElem)
                });
            }
    ...
    var selectImage = function() {
        x2.listingImageDialog = $( '#listing-image-dialog').dialog({
            dialogClass: 'no-close',
            buttons: [
            {
		id: 'img-button',
                text: 'Create w/o image',
                click: function() {GetridButtons();}
            },
            {
                text: 'Cancel',
                click: function() {
    ...
    x2.form = $('#Listings2-form');
    x2.form.submit(function(evt) {
        var industry = $('#Listings2_c_category__c');
        var params = industry.val();
        if ($('#dynamic_subcategory_dropdown_1').find(':selected').text())
            params.push($('#dynamic_subcategory_dropdown_1').find(':selected').text());
        if ($('#dynamic_subcategory_dropdown_2').find(':selected').text())
            params.push($('#dynamic_subcategory_dropdown_2').find(':selected').text());
        if ($('#dynamic_subcategory_dropdown_3').find(':selected').text())
            params.push($('#dynamic_subcategory_dropdown_3').find(':selected').text());
        if ($('#dynamic_subcategory_dropdown_4').find(':selected').text())
            params.push($('#dynamic_subcategory_dropdown_4').find(':selected').text());
        console.log(params);
        evt.preventDefault();
        if(!$('#Listings2_c_website_listing_image__c').val() || !$('#Listings2_c_listing_image__c').val()) {
	...
	var GetridButtons = function (){
		x2.form.unbind('submit').submit();
                $('body').css('cursor', 'progress');
                $('#save-button').hide();
                if ( $( '#img-button' ).length ) {
...
Yii::app()->clientScript->registerScript('placeMap', "
  ...
  console.log(map);
  $('.formSection:nth-child(7)').append($('#map'));
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyANDvDG3JnMJqJPtKFm4eoM6MAZu46LH1M&libraries=places&callback=initMap"
        async defer></script>
<div id="listing-image-dialog" title="Please Select a Listing Image"></div>
