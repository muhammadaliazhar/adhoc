...
<?php
    Yii::app()->controller->renderPartial('quickSetupInPersonRecipients',array(
        'envelope' => $envelope,
        'user' => User::getMe(),
...
<form class="host-information-form mt-3 hide">
    <div class="form-group">
        <label for="name">Host Name: </label>
        <input class="form-control" type="text" id="name" name="name" style="background: white !important;" placeholder="Host Name"required>
    </div>

    <div class="form-group mb-0">
        <label for="email">Host Email: </label>
        <input class="form-control" type="email" id="email" name="email" placeholder="Host Email" required>
    </div>
</form>
    ...
    */
    ...
    $(document).ready(function() {
        $(".host-information-form").dialog({
            ...
            position: { my: "center", at: "center", of: window },
            dialogClass: "host-information-dialog",
            buttons: [
                {
                    text: 'Save',
                    click: function () {
                        saveHostInformation($(this), <?php echo $envelope->id ?>);
                    },
                    class: 'save-btn'
                },
                {
    ...
    function saveHostInformation(event, envelopeId) {
        const form = $(".host-information-form")[0];
        ...
        $.ajax({
            url: yii.scriptUrl + '/x2sign/saveHostInfo?id=' + envelopeId,
            type: 'POST',
            data,
            success: function (response) {
                if (isValidJson(response)) {
                    ...
                    event.dialog('close');
                    showAlert('success', 'Host Information Saved Successfully');
                }
            },
            error: function (data) {
                showAlert('error', 'Some error occured while saving the Host Information');
            }
        });
    }
