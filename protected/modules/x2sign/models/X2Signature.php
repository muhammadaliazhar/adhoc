...
class X2Signature extends CActiveRecord {
    ...
    public $original_image = 'temp_signature.png';
    public $template_path = 'modules/x2sign/assets/main/';
    public $signature_folder = 'modules/x2sign/sign_folder/';
    /**
    ...
    public function regenSigImage() {
        $font = 0;
        $template_image = Yii::app()->getBasePath() . '/' . $this->template_path . $this->original_image;
        $im = imagecreatefrompng($template_image);
        $file_name = $this->signature_folder . str_replace(' ', '', 'JG') .
                        '_' . 'Contacts' . '_' . '594140' . '.png';
        if($im){
                list($width) = getimagesize($template_image);
                $text_box = imagettfbbox(50, 0, Yii::app()->getBasePath() . '/' . $this->template_path . $this->font[$font], 'JG');
                ...
                imagettftext($im, 50, 0, $corr_pos, 60, $black, Yii::app()->getBasePath() . '/' . $this->template_path . $this->font[$font], 'JG');
                if (function_exists('imageresolution')) imageresolution($im, 144);
                imagepng($im, Yii::app()->getBasePath() . '/' . $file_name, 9);
        }
    }
     ...
     * @returns string (path)
     */
    public function createImage($name, $modelType, $modelId, $initial = 0, $font = 0){
        ...
        if(!isset($signature)) {
            //echo "Signature not set\n";
            $template_image = Yii::app()->getBasePath() . '/' . $this->template_path . $this->original_image;
            $im = imagecreatefrompng($template_image);
            if($im){
                list($width) = getimagesize($template_image);
                $text_box = imagettfbbox(50, 0, Yii::app()->getBasePath() . '/' . $this->template_path . $this->font[$font], $name);
                $text_box_width = (abs($text_box[0]) + abs($text_box[2]));
                if($width > $text_box_width) {
                    $corr_pos = ($width - $text_box_width) / 2;
                } else {
                ...
                $background = imagecolorallocate($im, 255, 255, 255);
                imagecolortransparent($im, $background);
                //we are going to change the text size if the signiture is super long
                $textSize = 50;
                if(strlen($name) > 17){
                    $textSize = 30;
                    $corr_pos = 0;
                }
                imagettftext($im, $textSize, 0, $corr_pos, 60, $black, Yii::app()->getBasePath() . '/' . $this->template_path . $this->font[$font], $name);
                if (function_exists('imageresolution')) imageresolution($im, 144);
                imagepng($im, Yii::app()->getBasePath() . '/' . $file_name, 9);
            }
    ...
    public function getSignatureImage() {
        $imagePath =  Yii::app()->getBasePath() . '/' . $this->path;
        $pathCheck = strpos(addslashes($this->path),addslashes("uploads"));
        if( $pathCheck != FALSE) $imagePath = $this->path;
        $image = base64_encode(file_get_contents($imagePath));
        $src = 'data: ' . mime_content_type($imagePath ) . '; base64, ' . $image;
