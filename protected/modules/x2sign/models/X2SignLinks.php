 ...
 */
class X2SignLinks extends CActiveRecord {
	 ...
	 */
	public function tableName() { return 'x2_sign_links'; }
	...
	public function getRecipient() {
		return $this->modelType::model()->findByPk($this->modelId);
	}

	public function getKey() {
		if (empty($this->key)) {
			$this->key = bin2hex(random_bytes(18));
			if (self::countByAttributes(['key'=>$this->key])) {
				// assume it was a fluke and regenerate
				$this->key = bin2hex(random_bytes(18));
				if (self::countByAttributes(['key'=>$this->key])) {
					// once is an accident, twice is a pattern
					throw new Exception ('failed X2SignLink key collision check');
				}
			}
		}
		return $this->key;
	...
	public function beforeSave() {
        $this->getKey();
        if(empty($this->modelName) && !empty($this->modelType)){
            $this->setModelName();
        }
    ...
    public function setModelName(){
        $mainModel = $this->modelType::model()->findByPk($this->modelId);
        if(!empty($mainModel)){
            $this->modelName = $mainModel->name;
