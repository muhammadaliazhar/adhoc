...
Yii::app()->clientScript->registerScript('retrieveDocs', '
        ...
        if(fields.length > 0) {
            for(i = 0; i < fields.length; i++) {
                ...
                if(!field["id"].includes("Date") && (' . $position . ' == 2 || (' . $position . ' == 1 && field["recip"] == 1))) {
                   ...
                   //check if we have the text limt value
                    if(jQuery.inArray("maxlength", field)){
                        clone.find("input").attr("maxlength", field["maxlength"]);  
                    }            
                    ...
                    if(input.val() != null && input.val().length > 0) {
                        ...
                        var textLe =  getTextWidth($(clone).find("input").val() + "test", $(clone).find("input").css("font-size") + " " +  $(clone).find("input").css("font-family") );
                        var singleText =  getTextWidth("t", $(clone).find("input").css("font-size") + " "  +  $(clone).find("input").css("font-family") );
                        console.log($(clone).find("input").css("font-size"));
                        ...
                        if( textLe + (singleText * 2) + 4 + whiteLength> $(clone).width()){ 
                            $(clone).find("input").width(textLe - singleText  +  "px"); 
                            $(clone).width(textLe - singleText  +  "px");
                        }else if(textLe < $(clone).width() - (singleText * 2)){
                            $(clone).find("input").width(textLe - singleText  +  "px"); 
                            $(clone).width(textLe - singleText  +  "px");
    ...
    $(document).on("input", ".x2-sign-input", function() {
        ...
        if( textLe + (singleText * 2) + 4 + whiteLength> $(this).parent().width()){ 
            $(this).width(textLe - singleText  +  "px"); 
            $(this).parent().width(textLe - singleText  +  "px");
        }else if(textLe < $(this).parent().width() - (singleText * 2)){
            $(this).width(textLe - singleText  +  "px"); 
            $(this).parent().width(textLe - singleText  +  "px");
        ...
        //        $(this).width(FontSize + 5 + "px");
        //        $(this).parent().width(FontSize + 5 + "px");
        //    }
        //} else if($(this).parent().attr("id").indexOf("Text") == 0 && $(this).val().length < 4) {
        //    var holds = getTextWidth("test", $(this).css("font-size") + " Arial" );
        //    $(this).width($(this).css("font-size") * 5 + "px");
        //    $(this).parent().width($(this).css("font-size") * 5 + 2 + "px");
        //}
    });
...
<div id="create-signature-modal">
    <div class="modal-content">
        <p style="text-align:center;">In order for you to sign this document, you need to create a digital signature. To do so, please fill out your
        name in the field below.</p>
        <input id="create-signature" type="text"></input>
        </br>
        <p style="text-align:center; color:blue;">Choose a Font!</p>
        <div id="signature-font">
            <div class="font-choice"><input type="radio" name="font" value="0" checked><label class="font-Allura font-label">Allura</label></div>
            <div class="font-choice"><input type="radio" name="font" value="1"><label class="font-Otto font-label">Otto</label></div>
            <div class="font-choice"><input type="radio" name="font" value="2"><label class="font-May font-label">May-Queen</label></div>
        </div>
        <?php if(!Yii::app()->user->isGuest) { ?>
        <hr>
        <p style="text-align:center; padding-top:1em;color:blue;">Or Instead Upload Your Signature!</p>
        <div class="signature-upload">
            <?php $this->widget('FileUploader', array(
                    'id' => 'sign-upload',
                    'mediaParams' => ['signer_email' => $emailAddress, 'signer_upload' => 1, 'associationType' => 'contacts', 'associationId' => $model->id],
                    'viewParams' => ['showButton' => false, 'closeButton' => false],
                    'acceptedFiles' => 'image/*',
                ));
            ?>
            <center><button onclick="upload_signature()">Upload</button></center>
        </div>
        <?php } ?>
        <input class="create-signature-button x2-button" type="submit" value="Create signature"></input>
    </div>
</div>
...
<div id="document" class="column">
    ...
    <div style="text-align: center;">
        <span style="text-align: center;">Page: <span class="page_num"></span> / <span class="page_count"></span></span>
    </div>
    ...
    <div class="cell" id="cancel-button-container">
        <input id="cancel-signing-button" type="submit" class="x2-button" value="<?php echo Yii::t('app', 'Cancel');?>" />
    </div>
</div>
