...
class User extends X2Model {
    ...
    public function rules() {
        ...
        $userRules = array(
            ...
            array('username', 'match', 'pattern' => '/^[a-zA-Z0-9_\-\.@]+$/'), // Username must be alphanumerics/underscores only
            //array('username', 'match', 'pattern' => '/^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})$/'), // Username must be emails only
            //array('username', 'match', 'pattern' => '/^(([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})|(\w+))$/'), // Username must be emails only
    ...
    /* Sydney Customization - PETER CZUPIL - 10-19-2023 */
    /**
     * Gets id of role to which this user's group belongs
     * 
     * @return array
     */
    public function getRoleId() {
        /* $groupId = $this->getGroupIds();
        $results = Yii::app()->db->createCommand()
                ->select('roleId')
                ->from('x2_role_to_user')
                ->where('userId=:id AND type="group"', array(':id' => $groupId))
                ->queryAll();
        return $results; */
