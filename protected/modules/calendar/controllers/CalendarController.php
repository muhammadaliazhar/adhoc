...
class CalendarController extends x2base {
    ...
    public function actionOutlookSync () {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS,
            http_build_query(array('code' => $code,
                                   ...
                                   'redirect_uri' => 'https://dedupe.sydney.x2developer.com/index.php/calendar/outlooksync',
        )));
        ...
        if (isset($server_output)) {
            ...
            $user = Yii::app()->user;
            $userCalendar = X2Calendar::model()->findByAttributes(array('createdBy' => $user->getName()));
            ...
            $url = Yii::app()->createUrl('/calendar/update', array('id' => $userCalendar->id));
            $this->redirect($url);
        } else {
            $this->redirect('index');
        }
    ...
    public function actionUnsyncOutlook() {
        $user = User::getMe();
        $calendar = X2Calendar::model()->findByPk($_POST['id']);

        // If unsyncing the calendar, delete any events that were synced with Outlook
        $actions = Actions::model()->findAllByAttributes(array('calendarId' => $calendar->id));
        foreach($actions as $action) {
            $actionMetaData = ActionMetaData::model()->findByAttributes(array('actionId' => $action->id));
            if($action->type == 'event' && !empty($actionMetaData->etag) && !empty($actionMetaData->remoteCalendarUrl))
                $action->delete();
        }

        $profile = Profile::model()->findByAttributes(array('username' => $user->username));
        if (isset($profile)) {
            $calendar->remoteCalendarUrl = null;
            $calendar->remoteCalOutlook = null;
            $profile->outlookRefreshToken = null;
            $calendar->save();
            $profile->save();
        }
    ...
    public function actionDeleteAction(){
        if(isset($_POST['id'])){
            ...
            $action = Actions::model()->findByPk($id);
            $me = User::getMe();
            $calendar = X2Calendar::model()->findByAttributes(array('createdBy' => $me->username));
            if($action->type == 'event' && !empty($calendar->id) && !empty($calendar->remoteCalendarUrl)) {
                $calendar->attachSyncBehavior();
                if($calendar->asa('syncBehavior'))
                    if($calendar->syncType == 'outlook')
                        $calendar->deleteCalObject($action);
            }
