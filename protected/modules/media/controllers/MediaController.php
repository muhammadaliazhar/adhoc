...
class MediaController extends x2base {
    ...
    public function actionDelete($id) {
        if (Yii::app()->request->isPostRequest) {
            ...
            $doc = Docs::model()->findByAttributes(array('mediaId' => $model->id));
...
//    }
     ...
     * Renders preview
     */
    public function actionPreview($id, $key = null) {
        $model = X2Model::model('Docs')->findByAttributes(array('id' => $id));
        $model = $this->loadModel($model->mediaId);
        if (!$this->checkPermissions($model, 'view') && $key !== $model->getAccessKey()) {
            $this->denied();
        }
        $model->renderFile();
