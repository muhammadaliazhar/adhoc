        ...
        if(!Yii::app()->user->isGuest){
            if(empty($data->type) || $data->type == 'weblead'){
                if($data->complete == 'Yes' && 
                   Yii::app()->user->checkAccess('ActionsUncomplete',
                    ...
                    'ActionsComplete',array('assignedTo'=>$data->assignedTo)) || $data->assignedTo == User::getMe()->username ){
            ...
            if($data->type != 'workflow'){
                if(Yii::app()->user->checkAccess(
                    'ActionsUpdate',array('assignedTo'=>$data->assignedTo)) || $data->assignedTo == User::getMe()->username){
                ...
                if(Yii::app()->user->checkAccess(
                    'ActionsDelete',array('assignedTo'=>$data->assignedTo)) || $data->assignedTo == User::getMe()->username){
...
if($type == 'attachment' && $data->completedBy != 'Email') {
    ...
    echo 'Buyer has been verified by ' . $data->assignedTo;
} elseif ($data->type == 'buyerunverify') {
    echo 'Buyer has been un verified by ' . $data->assignedTo;
} else {
...
<?php } ?>
    ...
    <div class="footer">
        ...
        if (Yii::app()->settings->googleIntegration && isset($data->location) && !is_string($data->location)) {
