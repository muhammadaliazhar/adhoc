...
class X2RowsAndColumnsReport extends X2Report {
	...
	//Justin
        $permissionRow = $this->getPermissionsCondition($qpg);
	    ...
	    //Justin's Changes
            $pramsRow = $qpg->getParams();
            if( $this->permissionReport ){
                $newPrams = [];
                        foreach( $pramsRow as $key => $value ){
                         if( strpos( $key, ":generateRowsAndColumnsReport" ) !== false) {
                         $newPrams[$key] = $value;
                        }
                 }
            $pramsRow = $newPrams;
            ...
            $records = $query->queryAll (true, $pramsRow);
        } else {
            $records = array ();
        }
        ...
        for ($i = 0; $i < $rowCount; $i++) {
            ...
            foreach ($row as $col => &$val) {
                ...
                if ($dateFn = $this->getDateFn ($fns)) {
                    ...
                    $val = strip_tags($colModel->renderAttribute ($colAttr, false, true, false)); //remove html and other tags
                }
                $j++;
            }
        }
