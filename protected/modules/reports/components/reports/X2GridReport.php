...
class X2GridReport extends X2Report {
    ...
    public function generate ($render=true, $columnFilter=null) {
       ...
       //AuxLib::debugLogR ('$formattedColumnHeaders = ');
		...
		//for geting rid of id in name id
		  foreach($reportRecords as $key => $record){
                        if(isset($record['c_employee__c'])) $reportRecords[$key]['c_employee__c'] = substr($record['c_employee__c'], 0, strrpos( $record['c_employee__c'], '_'));
    ...
    protected function getFormattedColumnHeaders ($columns, $columnFilter=null) {
        ...
        foreach ($columns as $col) {
            ...
            if ($fns === array () ||  $col === self::NULL_ALIAS || $col === self::EMPTY_ALIAS) {
                $formattedColumnHeaders[$col] = strip_tags($columnAttrModel->renderAttribute (
            ...
            foreach ($row as $col => &$val) {
                if ($j === 0 && $i !== $rowCount - 1) {
                    ...
                    if ($rowDateFn) {
                        ...
                        $val = strip_tags($rowFieldModel->renderAttribute ($rowAttribute, false, true, false));
                    }
                } else if ($j !== 0 && $this->cellDataType !== 'count') {
                    $cellDataFieldModel->$cellDataAttribute = $val;
                    $val = strip_tags($cellDataFieldModel->renderAttribute (
