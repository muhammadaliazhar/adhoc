...
class X2SummationReport extends X2Report {
    ...
    /**
     * If true, generate drilldown sub grid, otherwise generate group headers grid
    ...
    public function getData (
        array $columns=array (), array $rows=array (), $enableDrillDown=false, $id=null) {
	//Justin change ^
    ...
    public function generateSubgrid () {
        ...
        $whereClause = 'WHERE '.$this->buildSQLConditions (array (
            ...
            array (
               $this->getRelitiveFilterConditions ('any', $relArray, $qpg), 'AND',
            ),
            array (
	...
	//Justin's Change
	$new_command = "SELECT t.assignedTo AS 'assignedTo', count(*) AS 'count(*)' FROM
        x2_listings2 AS t WHERE t.c_client_status__c = 'Active' AND t.createDate>=1530487720
        GROUP BY t.assignedTo ORDER BY count(*) DESC LIMIT 10";
	 ...
	 $relArray = $this->RelitiveFilters;
        ...
        $permission = $this->getPermissionsCondition($qpg);
	...
	//Justin's Changes
        ...
        $newPrams = [];
                foreach( $prams as $key => $value ){
                        if( strpos( $key, ":generateSummationReport" ) !== false) {
                        $newPrams[$key] = $value;
                        }
                }
        $prams = $newPrams;
        ...
        // don't fetch associative since field names might repeat (as a result of a join)
        $groupHeaders = $query->queryAll (true, $prams);
	
	//Justin's Change
	$id = CJSON::decode($id);
	if($id === "183"){
	$query = Yii::app()->db->createCommand($new_command);
	$groupHeaders = $query->queryAll(true);
	}
		...
		foreach($records as $key => $record){
			if(isset($record['c_employee__c'])) $records[$key]['c_employee__c'] = substr($record['c_employee__c'], 0, strrpos( $record['c_employee__c'], '_'));
			if(isset($record['assignedTo'])) {
                           $usersName = Yii::app()->db->createCommand()
                                ->select('firstName, lastName')
                                ->from('x2_users')
                                ->where('username = :username', array(':username' => $record['assignedTo']))
                                ->queryRow();
                           $records[$key]['assignedTo'] = $usersName['firstName'] . ' ' . $usersName['lastName'];
                        }
        ...
        for($i = 0; $i < count($records); $i++) {
            foreach($temp[$i] as $key => $value) {
                if(strpos($key, 'month') !== false) {
                    ...
                    // If our time format is of Month Day, Year,
            ...
            for ($i = 0; $i < $rowCount; $i++) {
                ...
                foreach ($row as $colAttr => &$val) {
                    ...
                    if (isset ($colModelsAndAttrs[$j]) && count ($colModelsAndAttrs[$j])) {
                        ...
                        $val = $colModel->strip_tags(renderAttribute ($colAttr, false, true, false));
                    }
                    $j++;
                }
            }
