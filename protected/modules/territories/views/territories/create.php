...
include("protected/modules/territories/territoriesConfig.php");
...
function initMap() {
  ...
  autocomplete.addListener('place_changed', function() {
    ...
    $('#Territories_c_Country').val('');
    $('#Territories_c_State').val('');

    // Fill geographic fields
    var streetAddr = '';
    place.address_components.forEach(function(addrComp){switch(addrComp.types[0]) {
            case 'country':
                 $('#Territories_c_Country').val(addrComp.long_name);
                 break;
            case 'administrative_area_level_1':
                 $('#Territories_c_State').val(addrComp.long_name);
                 break;
            default:
        }
    });

    $('#Territories_c_Territory_Latitude').val(place.geometry.location.lat());
    $('#Territories_c_Territory_Longitude').val(place.geometry.location.lng());
...
Yii::app()->clientScript->registerScript('placeMap', "
  var map = $('#googlemap');
  $('.formSectionRow').append($('#googlemap'));
...
?>
<div class="page-title territories"><h2><?php echo Yii::t('module','Create New {X}',array('{X}'=>Modules::itemDisplayName())); ?></h2></div>
