...
class Login_noticesController extends x2base {

    public $modelClass = 'Login_notices';
    ...
    public function actionView($id) {
        ...
        //set a time of about a year and half ago 
        $yearTimeAgo = strtotime("-2 year", time());
        $sql = 'SELECT * FROM x2_users WHERE id NOT IN ( SELECT firstId FROM x2_relationships WHERE secondType ="custom_Login_notices" AND secondId = '.$model->id.')'
            . ' AND login > ' . $yearTimeAgo;
        if (isset($_GET['notSeenByUserDataProvider_sort']) && isset(explode('.', $_GET['notSeenByUserDataProvider_sort'])[1])) {
            $orderArr = explode('.', $_GET['notSeenByUserDataProvider_sort']);
            $column = $orderArr[0];
            $order = $orderArr[1];
        ...
        $notSeenbyUser = $filtersForm->filter($rawData);

        $this->render('view', array(
            'model' => $model,
            'type' => $type,
            'notSeenbyUser' => $notSeenbyUser,
    ...
    public function actionCreate() {
        $model=new Login_notices;
        $users=User::getNames();

        if(isset($_POST['Login_notices'])) {
            $temp = $model->attributes;
            $model->setX2Fields($_POST['Login_notices']);
    ...
    public function actionIndex() {
        $model=new Login_notices('search');
    ...
    protected function performAjaxValidation($model) {
        if(isset($_POST['ajax']) && $_POST['ajax']==='login_notices-form') {
    ...
    public function actionLists() {
        ...
        $criteria->addCondition('modelName = "Login_notices"');
        ...
        $totalLogin_notices = X2Model::model('Login_notices')->count();
        $totalMyLogin_notices = X2Model::model('Login_notices')->count('assignedTo="' . Yii::app()->user->getName() . '"');
        $totalNewLogin_notices = X2Model::model('Login_notices')->count('assignedTo="' . Yii::app()->user->getName() . '" AND createDate >= ' . mktime(0, 0, 0));

        $allLogin_notices = new X2List;
        $allLogin_notices->attributes = array(
            'id' => 'all',
            'name' => Yii::t('contacts', 'All {module}', array('{module}' => Modules::displayName())),
            'description' => '',
            'type' => 'dynamic',
            'visibility' => 1,
            'count' => $totalLogin_notices,
            'createDate' => 0,
            'lastUpdated' => 0,
        );
        $newLogin_notices = new X2List;
        $newLogin_notices->attributes = array(
            'id' => 'new',
            'assignedTo' => Yii::app()->user->getName(),
            ...
            'type' => 'dynamic',
            'visibility' => 1,
            'count' => $totalNewLogin_notices,
            'createDate' => 0,
            'lastUpdated' => 0,
        );
        $myLogin_notices = new X2List;
        $myLogin_notices->attributes = array(
            'id' => 'my',
            'assignedTo' => Yii::app()->user->getName(),
...
//                    ),
    ...
    // Shows login_notices in the specified list
    ...
    public function actionCreateList($ajax = false) {
        $list = new X2List;
        $list->modelName = 'Login_notices';
        $list->type = 'dynamic';
        $list->assignedTo = Yii::app()->user->getName();
        $list->visibility = 1;

        $templateModel = new Login_notices;
        ...
        if (isset($_POST['X2List'])) {
            ...
            if (!$list->hasErrors() && $list->save()) {
                ...
                $this->redirect(array('/login_notices/login_notices/list', 'id' => $list->id));
            }
        }
    ...
    public function actionDeleteList() {
        ...
        $this->redirect(array('/login_notices/login_notices/lists'));
    }
