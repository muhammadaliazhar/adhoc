...
class ContactsController extends x2base {
    ...
    public function actionView($id) {
		...
		$envelopeId = $_GET['envelopeId'];
                ...
                if (isset($foundTemplate)) {
                    $docusign->createTemplate($envelopeId, 'Contacts');
                }
            }

        if ($this->checkPermissions($contact, 'view')) {
            if (isset($_GET['docusign']) && !empty($_GET['docusign'])) {
                if ($_GET['docusign'] === 'default') {
                    $default = Media::model()->findByAttributes(array('name'=>'Buyer_Profile_with_White_Template.docx'));
                    $doc = array(
                        'model' => $default,
                        'viewName' => $default->name,
                        'viewText' => $default->name,
                    );
                    echo CJSON::encode(array(array($doc)));
                } else if ($_GET['docusign'] === 'documents') {
                    $par = $_GET['query'];
                    ...
                    $library = Libraries::model()->findByAttributes(array('name'=>$franchisee->name));
                    $libraryCond = $library ? ' OR (template is not null AND libraryId = ' . $library->id . ')' : '';
		    //$docList = Media::model()->findAllBySql('select * from x2_media where (uploadedBy = "' . User::getMe()->username . '" or sharedTo like "%\"' . User::getMe()->username . '\"%") and fileName <> "" and name <> "" and name like "%' . $par . '%" and name not like "ReplacedFields_%" and name not like "Signed_%" and name not like "%.jpg" order by name asc');
                    //$docList = Media::model()->findAllBySql('select * from x2_media where ' . $libraryCond . ' (uploadedBy = "' . User::getMe()->username . '" or sharedTo like "%\"' . User::getMe()->username . '\"%") and fileName <> "" and name <> "" and name like "%' . $par . '%" and name not like "ReplacedFields_%" and name not like "Signed_%" and name not like "%.jpg"');
                    $docList = Media::model()->findAllBySql('select * from x2_media where sharedTo like "%\"' . User::getMe()->username . '\"%" OR (template is not null AND' . ' uploadedBy = "' . User::getMe()->username . '")' . $libraryCond . ' ORDER BY name ASC');
                    $docs = array();
                    foreach ($docList as $doc) {
		//want to return only docs with docusign templates
                        //will check to see if there is a relationships to see if templates
                        $rel1 = Relationships::model()->findByAttributes(array('firstType'=>'Media','firstId'=>$doc->id , 'secondType'=>'Docusign_templates'));
                        $rel2 = Relationships::model()->findByAttributes(array('secondType'=>'Media','secondId'=>$doc->id , 'firstType'=>'Docusign_templates'));


                        if ($doc->getPath() && (isset($rel1) ||  isset($rel2))) {
                            $docs[] = array(
                                'model' => $doc,
                                'viewName' => $doc->name,
                                'viewText' => $doc->name,
		...
		$me = User::getMe();
                $groupToUser = GroupToUser::model()->findByAttributes(array('userId' => $me->id));
                $nda = Media::model()->findByPk(200118573);
                $documents = array();
                if (isset($nda) && !empty($nda)) {
                    $documents[] = $nda;
                }
                if (empty($documents) && isset($groupToUser) && isset($groupToUser->groupId)) {
                    $group = Group::model()->findByPk($groupToUser->groupId);
                    if ((isset($group) && isset($group->nda))) {
                        $groupNda = Media::model()->findByPk($group->nda);
                        $documents[] = $groupNda;
                    }
                }
                $recipients = array();
                $recipients[] = $contact;
                $docusign = DocusignBehavior::createDocusignInstance();
                echo $docusign->requestAccess($documents, $recipients, "Buyer Profile- Action required", $contact->id);
                Yii::app()->end();
            }
            // If document sent
            if (isset($_POST['recipients']) && !empty($_POST['recipients'])) {
		$Asso_Listing = $contact->c_listinglookup__c;
		$emailSubject = isset($_POST['emailSubject']) ? $_POST['emailSubject'] : '';
                $emailSubLine = isset($_POST['emailSubLine']) ? $_POST['emailSubLine'] : '';
		...
		foreach ($documentList as $documentId) {
                ...
                $docusign = DocusignBehavior::createDocusignInstance();
                echo $docusign->requestAccess($documents, $recipients, $emailSubject, $contact->id, $emailSubLine, $Asso_Listing);
            ...
            if ($contact->checkForDuplicates() && Yii::app()->params->isAdmin) {
    ...
    public function actionGetItemsSubCategories(){
        ...
        LinkableBehavior::getItems ($term, 'name', 'id', null,$categoryTerms, true);      
    }
...
<br />" . Yii::t('app', 'Link') . ": " . CHtml::link($model->name, $this->createAbsoluteUrl('/contacts/contacts/view', array('id' => $model->id)));
	...
	$me = User::getMe();
        if (isset($_POST['Contacts'])) {
            ...
            $shareArray = array(); 
	    if(isset($_POST['Contacts']['sharedTo']))
	         $shareArray = $_POST['Contacts']['sharedTo'];
            $uIDs = array();
	    ...
	    $model->c_createdbyid = User::getMe()->username;
            $model->setName();
	    if(isset($model->c_listing_number_lookup)) $listing = Listings2::model()->findByAttributes(array('c_listing_number__c' => $model->c_listing_number_lookup));
	    if(isset($listing)) $model->c_listing_number_lookup = $listing->nameId;
            if (isset($_POST['x2ajax'])) {
                $ajaxErrors = $this->quickCreate($model);
            } else {
                if ($model->validate() && $model->checkForDuplicates() && Yii::app()->params->isAdmin) {
	...
	$ContactsThatAreClones = Contacts::model()->findAll( $q );     // works!
	$cloneCount = count($ContactsThatAreClones)+1;
    ...
    public function actionMobileCreatePost($atts){
        ...
        $model->save();
        $this->redirect(array('contacts/view','id'=>$model->id));
    ...
    public function actionMobileUpdate($atts){
        ...
        foreach($attsArray as $key => $value){
            if($key == 'id') continue;
            $contactsArray[$key] = $value;
            //$model->$key = $value;
        }
        $model->setX2Fields($contactsArray);
        $model->save();
        $this->redirect(array('contacts/view','id'=>$model->id));
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Contacts'])) {
            ...
            //fix for shared to update

            //only let admins pass value
            ...
            if(!isset($_POST['Contacts']['sharedTo'])){
                ...
                $shareArray = isset($_POST['Contacts']['sharedTo']) ? $_POST['Contacts']['sharedTo'] : array();
                $uIDs = array();
                foreach($shareArray as $uname){
                    $user = User::model()->findByAttributes(array('username' => $uname));
                    if(isset($user))$uIDs[] = $user->id;
...
//                    ),
    ...
    public function actionExportInquiryContacts() {
        // Export Contacts using Inquiry criteria
        if (isset($_GET['listingId'])) {
            $criteria = new CDBCriteria();
             $criteria->select = 't.firstName, t.lastName, t.phone, t.c_mobilephone, t.email, t.address, t.address2, t.city, t.state, t.zipcode, t.country, t.assignedTo, t.leadSource, t.createDate, t.c_lead_generated_by__c, t.c_stage__c, t.c_status__c ,t.c_listinglookup__c, t.c_ddp_listing_number__c, t.nameId, i.c_docusign_status';
            $criteria->join = 'JOIN x2_listings2 l JOIN x2_inquiries i ON t.nameId = i.c_contact__c AND l.nameId = i.c_listing__c';
            $criteria->addCondition('l.id = :listingId');
            $criteria->params = array(':listingId'=>$_GET['listingId']);

            // Only export what the user can access
            $conditions = Contacts::model()->getAccessConditions(4);
            ...
            $_SESSION['modelExportFile'] = User::getMe()->username . "_contact_inquiry_export.csv";
            $_SESSION['modelExportMeta'] = array_keys(X2Model::model('Contacts')->attributes);
    ...
    public function actionDownloadBM() {
        $file = '/var/www/transworld_crm/protected/data/buyerPref.csv';
        $export_csv = "SELECT * FROM x2_buyer_prefs";
        ...
        $file = '/var/www/transworld_crm/protected/data/buyerMatchHistory.csv';
        $export_csv = "SELECT * FROM x2_buyer_match_history";
    ...
    public function insertMenu($selectOptions = array(), $model = null, $menuParams = null) {
        ...
        $me = User::getMe();
        $groupToUser = GroupToUser::model()->findByAttributes(array('userId' => $me->id));
        $group = isset($groupToUser) ? Groups::model()->findByPk($groupToUser->groupId) : null;
        $roleToUser = isset($group) ? RoleToUser::model()->findByAttributes(array('userId' => $group->id, 'type' => 'group')) : null;
        $role = isset($roleToUser) ? Roles::model()->findByPk($roleToUser->roleId) : null;
        ...
        $menuItems = array(
            ...
            array(
                'name' => 'clone',
                'label' => Yii::t('contacts', 'Clone {module}', array('{module}' => $Contact)),
                'url' => array('clone', 'id' => $modelId),
            ),
            array(
                'name' => 'save',
            ...
            array(
                ...
                'visible' => ((bool) (Yii::app()->params->isAdmin || isset($role) && $role->id == 13))
            ),
            array(
                'name' => 'export',
                'label' => Yii::t('contacts', 'Export {module}', array('{module}' => $Contacts)),
                'url' => array('site/exportModels', 'model' => 'Contacts')
                //'visible' => ((bool) (Yii::app()->params->isAdmin || isset($role) && $role->id == 13))
            ),
            array(
                'name' => 'viewOnMap',
            ...
            array(
                'name' => 'downloadBuyerMatch',
                'label' => 'Download Buyer Match',
                'url' => 'downloadBM',
                'visible' => Yii::app()->params->isAdmin ? true : false,
            ),
            array(
                'name' => 'quick',
	...
	array(
	'name'=>'convert',
                'label' => Yii::t('x2Leads', 'Convert'),
		'url' => '#',
		'linkOptions' => array ('id' => 'convert-contact-button'),
	  ),
            array(
                'name' => 'print',
	...
	$confidential =  Yii::app()->request->getPost('confidential');
        if(isset($confidential) && $confidential === 'true') $docName = 'BLI_Confidential_Template.docx';
        $contact = $this->loadModel($id);
        ...
        echo $docusign->requestAccess(array($doc), array($contact), $subject, $id, null, null, true);
    }

    public function actionSendExc() { //CUSTOM JUSTIN JULY 2019
        $id = Yii::app()->request->getPost('id');
        $docName = "Exc_Waiver.docx"; //always this
        $contact = $this->loadModel($id);
        $docusign = DocusignBehavior::createDocusignInstance();
        $doc = Media::model()->findByAttributes(array('name' => $docName));
        $subject = "Confidential Exclusive Waiver For Business Profile - " . $contact->name;
        echo $docusign->requestAccess(array($doc), array($contact), $subject, $id, null, null, true);
    }

    public function actionPreviewBli() {
        $id = Yii::app()->request->getPost('id');
        $checked = Yii::app()->request->getPost('checked');
        $docName = $checked === 'true' ? 'SA-BLI.docx' : 'BLI_Template.docx';
        $contact = $this->loadModel($id);
        ...
        $doc = Media::model()->findByAttributes(array('name' => $docName));
        $subject = "Confidential Business Profile - " . $listing->c_listing_number__c;
        echo $docusign->requestPreview(array($doc), array($contact), $subject, $id, null, null, true);
    }

    public function actionUpdateBuyPrefs(){
        $down = str_replace(',', '', $_POST['down']);
        $price = str_replace(',', '', $_POST['price']);
        $minSell = str_replace(',', '', $_POST['minSell']);
        $cats = $_POST['cats'];
        $geos = $_POST['geos'];
        $buyId = $_POST['buyerId'];
        $country = $_POST['country'];
        $cats = json_decode($cats,TRUE);
        //these will be the states if we marked all on geo or categories
        $catsAll = false;
        $geoAll = false;
        //convert name to nameId
        foreach($cats as $key => $value){
            ...
            foreach($value as $sub){
                if($sub != 'all'){
                    $subRec = Subcategories::model()->findByAttributes(array("name" => $sub));
                    $newSubs[] = $subRec->nameId;
                }else{
                    $newSubs[] = '';
        ...
        //check geo
        $geoArray = json_decode($geos,TRUE);
        foreach($geoArray as $g){
            if($g == "all"){
                $geoAll = true;
                $geos = '["all"]';
            }
        ...
        //$cats = substr_replace($cats ,"", -1);
        //$cats = substr($cats, 1);
        //$cats = '{'.$cats.'}';
        $buyPrefId = Yii::app()->db->createCommand()
                ->select('down_payment, price , geography , industry_subcartegory')
                ->from('x2_buyer_prefs')
                ->where('buyerId = :buyerId', array(':buyerId' => $buyId))
                ->queryRow();


                //check to see if we update or create a record
        $buyPrefId = Yii::app()->db->createCommand()
                ->select('down_payment, price , geography , industry_subcartegory')
                ->from('x2_buyer_prefs')
                ->where('buyerId = :buyerId', array(':buyerId' => $buyId))
                ...
                //check to see if we update or create a record
               if(!empty($buyPrefId)){
                   $sql = "UPDATE x2_buyer_prefs SET down_payment = " .  $down . " , price = "
                    .  $price . " , geography = '"
                    .  $geos . "' , industry_subcartegory = '"
                    .  $cats . "' , minimum_seller_discretionary_earnings = '"
                    .  $minSell . "' , country =  '" . addslashes($country) . "' "
                        . " where buyerId = " . $buyId;
                    Yii::app()->db->createCommand($sql)->execute();

                }else{
                    $sql = "INSERT INTO x2_buyer_prefs (buyerId , down_payment , price , geography , industry_subcartegory, minimum_seller_discretionary_earnings, country) VALUES (" .
                        $buyId . ", " . $down . ", " . $price . ", '"
                     .  addslashes($geos) . "', '" . addslashes($cats) .  "' , '" .  addslashes($minSell)  . "' , '" . addslashes($country) . "')";
                    Yii::app()->db->createCommand($sql)->execute();

                }

         // Create Action if user updated prefs
         $action = new Actions;
         $action->associationId = $buyId;
         ...
         $action->actionDescription = Yii::app()->user->getName() . " has updated buyer preferences";
         $action->save();
    ...
    public function actionVerifyUser() {
        $result=Yii::app()->db->createCommand()->update('x2_contacts', array('c_verify_buyer' => 1), 'id=:id', array(':id' => $_POST['id']));
        // Associate verifybuyer action with buyer
        $me = User::getMe();
        $action = new Actions;
        $action->type = 'buyerverify';
        $action->actionText = 'Buyer has been verified by ' . $me->username;
        $action->createDate = $action->lastUpdated = time();
        $action->assignedTo = $action->updatedBy = $me->username;
        $action->associationType = 'Contacts';
        $action->associationId = $_POST['id'];
        $action->save();
    }
    // for Un-verifying the buyer
    public function actionUnVerifyUser() {
        $result=Yii::app()->db->createCommand()->update('x2_contacts', array('c_verify_buyer' => 0), 'id=:id', array(':id' => $_POST['id']));
        // Associate unverifybuyer action with buyer
        $me = User::getMe();
        $action = new Actions;
        $action->type = 'buyerunverify';
        $action->actionText = 'Buyer has been un verified by ' . $me->username;
        $action->createDate = $action->lastUpdated = time();
        $action->assignedTo = $action->updatedBy = $me->username;
        $action->associationType = 'Contacts';
        $action->associationId = $_POST['id'];
    ...
    // function for sending Bli
    public function actionSendBliID($id) {
        // The $id parameter is added for automatically send Bli after Nda has been signed.
        if(!isset($id)){
            $id = Yii::app()->request->getPost('id');
        }
        $checked = Yii::app()->request->getPost('checked');
        $docName = $checked === 'true' ? 'SA-BLI.docx' : 'BLI_Template.docx';
        $confidential =  Yii::app()->request->getPost('confidential');
        if(isset($confidential) && $confidential === 'true'){
            $docName = 'BLI_Confidential_Template.docx';
        }
        $contact = $this->loadModel($id);
        $listing = Listings2::model()->findByAttributes(array('nameId' => $contact->c_listinglookup__c));
        if (!$listing) {
            echo "No related listing found for this record";
            return;
        }
        $docusign = DocusignBehavior::createDocusignInstance();
        $doc = Media::model()->findByAttributes(array('name' => $docName));
        $subject = "New Listing BLI";
        echo $docusign->requestAccess(array($doc), array($contact), $subject, $id, null, null, true);
    }
