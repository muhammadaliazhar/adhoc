...
<div class="property-menu" style="padding-top: 10px; height:100%; border-radius: 30px 30px 0px 0px;">
    ...
    <div id="Details" class="tabcontent" style="margin-bottom: 150px;">
        ...
        function updateRecord(){
            var optionsToPass = [['Contacts_name','name'],['Contacts_email','email'],['Contacts_phone','phone'],['Contacts_assignedTo_assignedToDropdown','assignedTo'],['Contacts_c_mobilephone','c_mobilephone'],['Contacts_leadSource','leadSource'],['Contacts_c_listing_number_lookup_id0','c_listing_number_lookup'],['Contacts_c_referredBy','c_referredBy']];
            var passVars = {"id": <?php echo $model->id?> };
            optionsToPass.forEach(function(element){
                passVars[element[1]] = $("#" + element[0]).val();
                if(element[1] === "c_listing_number_lookup")passVars[element[1]] = $("#Contacts_c_listing_number_lookup").val() + "_" + passVars[element[1]].toString();
            })
            console.log(passVars);
            document.location.href='<?php echo Yii::app()->createUrl('/contacts/mobileUpdate');?>'+ '?atts='+encodeURIComponent(JSON.stringify(passVars));
