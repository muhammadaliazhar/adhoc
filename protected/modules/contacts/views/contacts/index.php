...
//if (Yii::app()->params->isAdmin) {
    $menuOptions = array(
        'all', 'lists', 'create', 'map', 'savedMaps', 'downloadBuyerMatch',
    );
...
//} else {
//    $menuOptions = array(
//        'all', 'lists', 'create', 'map', 'savedMaps',
//    );
//}
...
<?php
/*if($this->beginCache(User::getMe()->username.get_class($model) . "_index", array('dependency'=>array(
        'class'=>'system.caching.dependencies.CDbCacheDependency',
        'sql'=>'SELECT MAX(timestamp) FROM x2_changelog WHERE type="Contacts"')))) {*/
$this->widget('X2GridView', array(
    ...
    '{title}{buttons}{filterHint}{massActionButtons}'.
    X2Html::createNewButton() .
    '{summary}{topPager}' .
    '{items}{pager}',
    'fixedHeader' => true,
    ...
    'specialColumns' => array(
        ...
        'c_listing_number_lookup'=>array(
                'name'=>'c_listing_number_lookup',
                'header'=>Yii::t('app','Listing Number Lookup'),
                'value'=>'CHtml::link($data->c_listing_number,Yii::app()->controller->createAbsoluteUrl(\'/listings2/\' . substr($data->c_listinglookup__c, strpos($data->c_listinglookup__c, "_") + 1)))',
                'type'=>'raw',
        ),
    ),
    'massActions' => array(
