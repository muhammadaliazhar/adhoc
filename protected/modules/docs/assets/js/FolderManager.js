...
FolderManager.prototype._init = function () {
    var that = this;
    $('.file-system-object-doc a').attr('target', '_blank');
    ...
    $(document).on('click','.file-system-object-folder .folder-link',function(){
        ...
        var currDirSpan = $('#curr-dir-text')
        var currDir = currDirSpan.html();
        if ($(this).html() === '[BACK]') {
            console.log(currDir + ", index: " + currDir.lastIndexOf(' &gt; ')); // URL is encoded
            dirStr = currDir.substring(0, currDir.lastIndexOf(' &gt; '));
            currDirSpan.html(dirStr);
        } else {
            currDirSpan.html(currDir + " > " + $(this).html());
        }
        ...
        $.fn.yiiGridView.update('folder-contents',{
            ...
            complete:function(){
                ...
                window.history.pushState("Folder Id", "Folder Id", url);
                window.history.pushState("Folder Id", "Folder Id", window.location.href + "?folderId=" + num);
    ...
    /*x2transworldstart*/
    // Search Function
    $('#docs-search-bar').bind('enterKey', function() {
    ...
    $('#docs-search-bar').keyup(function(e){
        if(e.keyCode == 13){
