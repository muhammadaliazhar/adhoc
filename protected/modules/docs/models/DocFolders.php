...
class DocFolders extends CActiveRecord {
    ...
    public function getLibraryChildren($option = null, $libraryId = null) {
        ...
        if ($option === 'root') {
            ...
            file_put_contents('/tmp/getlibrarychildren.txt', CJSON::encode($option) . "\n" . $libraryId);
            $folderCriteria->condition = 'parentFolder IS NULL AND (id > 0 OR id = -2) AND libraryId = ' . $libraryId;
        } else {
        ...
        if ($option === 'root') {
            ...
            $folderCriteria->condition = 'libraryId = ' . $libraryId . ' AND type NOT IN ("email","quote")';
        ...
        $docsCriteria->condition = 'libraryId IS NOT NULL';
    ...
    public function getPath() {
        ...
        file_put_contents('/tmp/test-docusign.txt', json_encode($pathArr) . "\n", FILE_APPEND);
    ...
    public function getChildren($option = null) {
        $children = array('folders' => array(), 'docs' => array());
        $folderCriteria = new CDbCriteria();
        if ($option === 'root') {
            $folderCriteria->condition = 'parentFolder IS NULL AND id > 0';
    ...
    public function getRootLibraryContents( $pageSize=null, $id=null, $libraryId=null) {
        if (!$pageSize) $pageSize = $this->getPageSize ();
        $model = DocFolders::model();
        $contents = $this->findLibraryChildren ('corp', array('folder', 'doc'), array(), $libraryId);
        file_put_contents('/tmp/contents.txt', CJSON::encode($contents));
        return new FileSystemObjectDataProvider($contents, array(
            'id' => 'root-folder-contents',
            'pagination' => array(
                'pageSize' => $pageSize,
    ...
    public function findLibraryChildren ($folderId='root', array $types=array ('folder', 'doc'),
        array $exclude=array (), $libraryId = null) {
        //, $depth=1) {
        ...
        if ($folderId === 'root') {
            $model = self::model ();
            $fileSysObjs = array ($model->createFileSystemObject ('templates', null));
        } elseif ($folderId === 'corp') {
            $model = self::model ();
            $fileSysObjs = array ($model->createFileSystemObject ('Corporate', null));
        } elseif ($folderId === self::TEMPLATES_FOLDER_ID) {
            $model = self::model ();
            $this->id = self::TEMPLATES_FOLDER_ID;
            $fileSysObjs = array($this->createFileSystemObject ('parent', null));
        ...
        $children = $model->getLibraryChildren ($folderId, $libraryId);
        file_put_contents('/tmp/children.txt', CJSON::encode($children));
        foreach ($types as $type) {
            foreach ($children[$type.'s'] as $child) {
                $fileSysObjs[] = $this->createFileSystemObject ($type, $child);
            }
    ...
    private function createFileSystemObject($type, $object) {
        ...
        if ($type === 'parent') {
            ...
            $options['objId'] = -2;
            $options['type'] = 'folder';
            $options['name'] = 'Corporate';
            $options['title'] = $object ? $object->name : Yii::t('docs', 'Corporate');
            $options['isParent'] = false;
        } elseif ($type === 'templates') {
            $options['type'] = 'folder';
            $options['objId'] = -1;
            $options['name'] = Yii::t('docs', 'Templates');
            if (!empty($object->sharedTo))
                $options['sharedTo'] = $object->sharedTo;
        } else {
            $options['type'] = $type;
            $options['objId'] = $object->id;
            $options['name'] = $object->name;
            $options['createdBy'] = $object->createdBy;
            $options['assignedTo'] = $object->assignedTo;
            $options['sharedTo'] = $object->sharedTo;
