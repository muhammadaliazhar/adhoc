...
if (!$model->isNewRecord) {
    $existingRecordMenuOptions = array(
        'viewSignable', 'permissions', 'editSignable', 'delete'
    );
...
Yii::app()->clientScript->registerScript("retrievePdf", '
    ...
    function createDraggable(id) {
        ...
        var setHight = 10 + $(window).scrollTop();
        $(clone).css({
            "left": "0px", 
            "top": setHight, 
            "position": "absolute",
    ...
    $("#create-button").click(function(e) {
        ...
        if(!errors) {
            ...
            $.ajax({
                url: "' . $this->createUrl('/docs/docs/updateSignable/id') . "/" . $model->id . '",
                type: "POST",
                data: {
                    id: ' . $model->id . ',
                    name: $("#doc-name").val(),
    ...
    $("#Media_name").change(function() {
        ...
        $.ajax({
            url: "' . $this->createUrl('/x2sign/x2sign/getFile/id') . '/" + $("#MediaModelId").val(),
            type: "GET",
            success: function(data) {
                if(data == "fail") {
   ...
   var scrollTop = $(window).scrollTop(); // check the visible top of the browser  
    ...
    function showFields(fields) {
        $.each(fields, function(index, value) {
            ...
            var fieldNumber = parseInt(value["id"].substring(value["id"].indexOf("-") + 1)); 
            if(fieldNumber > draggableCounter) {
                draggableCounter = fieldNumber + 1;
            }
            ...
            if($(clone).children("input").attr("id") !== "checkbox")
                $(clone).children("input").css("width", value["width"] - 35).val(value["value"]);

            // Size the wrapper div to the draggable div
            $(clone).children("div").css("width", value["width"]);
            
            //check to see if we need to set the maxlength element
...
Yii::app()->clientScript->registerScript('setDocName', '
    $("#Media_name").val("' . $pdfName . '");

    pdfDoc = null,
        url = "' . $this->createUrl('/x2sign/x2sign/getFile/id') . '/" + "' . $signDoc->mediaId . '",
        pageNum = 1,
        pageRendering = false,
        pageNumPending = null,
        scale = 2,
    ...
    renderPDF(url);
    documentId = ' . $signDoc->mediaId . ';
    $(".add-fields").show();
    showFields(' . $signDoc->fieldInfo . ');
    ...
    <div class="row">
            <div class="cell">
                <?php
                    ...
                    $prePick = $model->libraryId;
                    if(empty($prePick))$prePick = '';
                    $officeCheck = false;
                    $employee = Employees::model()->findByAttributes(array('c_user__c'=>User::getMe()->username));
                    if(isset($employee)){
                        $office = Franchisees::model()->findByAttributes(array('nameId' => $employee->c_franchisee__c));
                        if(isset($office)){

                            $libraryModel = X2Model::model('Libraries')->findByAttributes(array('name' => $office->name));
                            if(isset($libraryModel)){
                                if($model->libraryId == $libraryModel->id)  $officeCheck = true;
                            }
                        }
                    ...
                    if(Groups::userHasRole($user->id, 'Franchise') || Yii::app()->params->isAdmin){
                        echo CHtml::label('Put in office library', '#officeLibrary') .  CHtml::checkbox('officeLibrary',  $officeCheck);
                    }
                    if(Yii::app()->params->isAdmin){
                        ...
                        echo CHtml::dropdownList('select-Library', $prePick,   $listopp , array('id' => 'select-Library'));


                        $sqlDocList = 'SELECT id, name FROM x2_sign_docs ORDER BY name ASC';
                        $command = Yii::app()->db->createCommand($sqlDocList);
                        $DocList = $command->queryAll();
                        $DocOpp = array(''=>'');
                        foreach( $DocList as $key => $value){
                            $DocOpp[$value['id']] = $value['name'];
                        }

                        echo  CHtml::label('Copy prexsiting Doc fields', '#copyDoc');
                        echo  CHtml::dropdownList('copy-doc', "",   $DocOpp , array('id' => 'copy-doc'));
    ...
    <div class="row" style="display: flex; flex-direction: row; justify-content: center;">
        <div id="field-bank" class="column" style="display: none;">
            ...
            <div id="Initials-draggable" class="draggable resizable">
                <a class="delete-field" onclick="deleteDraggable(this.parentNode);" href="#"><i class="fas fa-times-circle fa-lg"></i></a>
                <input id="initials" class="x2-sign-input" type="text" minlength="1" maxlength="5" size="5" placeholder="Initials" style="font-size:12px"></input>
            </div>
            ...
            <div id="Name-draggable" class="draggable resizable">
                <a class="delete-field" onclick="deleteDraggable(this.parentNode);" href="#"><i class="fas fa-times-circle fa-lg"></i></a>
                <input id="name" class="x2-sign-input" type="text" minlength="1" size="15" placeholder="Name" style="font-size:12px"></input>
            </div>
            ...
            <div id="Title-draggable" class="draggable resizable">
                <a class="delete-field" onclick="deleteDraggable(this.parentNode);" href="#"><i class="fas fa-times-circle fa-lg"></i></a>
                <input id="title" class="x2-sign-input" type="text" minlength="1" placeholder="Title" style="font-size:12px;"></input>
