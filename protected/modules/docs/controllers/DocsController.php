...
class DocsController extends x2base {
    ...
    public function actionView($id) {
        ...
        printR(Yii::app()->controller->createUrl('/media/media/view', array('id' => $model->mediaId)), true);

        if (isset($model->mediaId)) {
            $this->redirect(Yii::app()->controller->createUrl('/media/media/view', array('id' => $model->mediaId)));
            return;
        }
    ...
    public function actionIndex($id = null) {
        ...
        if (Yii::app()->request->isAjaxRequest && isset($_POST['DocFolders'])) {
            ...
            $condition = "associationType='docs' AND sharedTo LIKE '%\"".$currentUserUsername."%\"'";
            ...
            $i=0;
            foreach ($folderDataProvider->rawData as $data) {
                $folderId = $data->objId;
                $folderModel = DocFolders::model()->findByPk($folderId);
                $className = get_class($folderModel);
                $sharedToString='';
                if ($className==='DocFolders' && !strpos($folderModel->sharedTo, 'Anyone')) {
                    $sharedToString = $folderModel->sharedTo;
                    $enclosedUsername = '"'.$currentUserUsername.'"';
                    if (!strpos($sharedToString, $enclosedUsername)) {
                        unset($folderDataProvider->rawData[$i]);
                    }
                }
                $i++;
    ...
    public function insertMenu($selectOptions = array(), $model = null, $menuParams = null) {
        ...
        $menuItems = array(
            ...
            array(
                'name' => 'uploadDoc',
                'label' => Yii::t('docs', 'Upload {doc}', array(
                    '{doc}' => Modules::displayName(false, "Doc"),
                )),
                'url' => array('uploadDoc')
            ),
            array(
