...
class LibrariesController extends x2base {

    public $modelClass = 'Libraries';
    ...
    public function actionView($id) {
        ...
        $parentId = substr($model->c_parent_library, strpos($model->c_parent_library, "_") + 1);
        $media = $this->searchMediaLibrary($id, $parentId);
        /* $media = Media::model()->findAllByAttributes(array(
            'associationType' => 'Libraries',
            'associationId' => $id,
        ));
        $media = array_merge($media, Media::model()->findAllByAttributes(array(
            'associationType' => 'Libraries',
    ...
    public function actionCreate() {
        ...
        if(isset($_POST['Libraries'])) {
            ...
            $model->setX2Fields($_POST['Libraries']);
    ...
    public function actionUpdate($id) {
		...
		foreach ($selectedGroups as $aSelectedGroup) {
		    $record = Groups::model()->findByPk((int)$aSelectedGroup);
		    if (isset($record)) 
			$selectedGroupNames[] = $record->name;
		}
	        $model->sharedTo = json_encode($selectedGroupNames);
	    }
            $model->setX2Fields($_POST['Libraries']);
    ...
    public function actionIndex() {
        $model=new Libraries('search');
        //if (Yii::app()->params->isAdmin)
            $this->render('index', array('model'=>$model));

        //$this->render('userIndex', array('model'=>$model));
    }
    ...
    protected function performAjaxValidation($model) {
        if(isset($_POST['ajax']) && $_POST['ajax']==='libraries-form') {
            echo CActiveForm::validate($model);
            Yii::app()->end();
        }
    }

    private function searchMediaLibrary($id, $parentId) {
        $search = new CDbCriteria;
        $search->addCondition("associationType = 'Libraries' AND associationId = $id");
        if ($parentId)
            $search->addCondition("associationType = 'Libraries' AND associationId = $parentId", "OR");
        return new SmartActiveDataProvider('Media', array(
            'criteria' => $search,
            'sort' => array(
