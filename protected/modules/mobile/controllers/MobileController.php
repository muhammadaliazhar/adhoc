...
class MobileController extends X2Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array(/* x2entstart */'userMap', /* x2entend */ 'chat', 'logout', 'home', 'getMessages', 'newMessage', 'contact',
    ...
    public function actionUserMap() {
        $this->headerTitle = Yii::t('mobile', 'Nearby Contacts');

        $me = User::model()->getMe()->getRecentLogin();
        $locations = Locations::model()->getRecentRecords();

        if ($me) {
            $center = array('lat' => $me->lat, 'lng' => $me->lon);
        } else {
