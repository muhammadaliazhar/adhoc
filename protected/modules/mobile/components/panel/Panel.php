...
class Panel extends X2Widget {
	...
	$userMap = new PanelItem;
	$userMap->setTitle(Yii::t('app', 'Nearby Contacts'));
	$userMap->setId('userMap');
	$userMap->setIsSelected(preg_match (
            '/mobile\/userMap$/', Yii::app()->request->pathInfo));
	$userMap->setHref(Yii::app()->createAbsoluteUrl('/mobile/userMap'));

  	// TODO: fix hack

	$items = array_merge(array($userMap), $items);
	$temp = $items[0];
	$items[0] = $items[1];
	$items[1] = $temp;
