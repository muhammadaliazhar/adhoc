...
function EmailInboxesQtipManager (argsDict) {
    ...
    var defaultArgs = {
        ...
        translations: {
            Email: 'Email',
            'Create contact': 'Create buyer',
            'Create seller': 'Create seller',
            'Create generic contact': 'Create generic contact',
            'action': 'action'
        }
    };
...
EmailInboxesQtipManager.prototype._getNonContactConfig = function (elem, email, name) {
    ...
    var template$ = $(
        '<div>' +
            ...
            '</button>' +
            '<button class="new-seller-from-entity-button x2-button">' +
                this.translations['Create seller'] +
            '</button>' +
            '<button class="new-generic-contact-from-entity-button x2-button">' +
                this.translations['Create generic contact'] +
            '</button>' +
        '</div>');
...
EmailInboxesQtipManager.prototype._setUpQuickCreateButtonBehavior = function () {
    ...
    $(document).off ('click._setUpQuickCreateButtonBehavior', '.new-contact-from-entity-button').
        ...
        attributes.leadSource = 'Direct Mail';
        attributes.assignedTo = $.cookie('LoginForm[username]');
        new x2.QuickCreate ({
            modelType: 'Contacts',
    ...
    // close tooltip and open quick create dialog when quick create button is clicked
    $(document).off ('click._setUpQuickCreateButtonBehavior', '.new-seller-from-entity-button').
        on ('click._setUpQuickCreateButtonBehavior', '.new-seller-from-entity-button',
            function () {
        ...
        attributes.c_leadsource = 'Direct Mail';
        attributes.assignedTo = $.cookie('LoginForm[username]');
        new x2.QuickCreate ({
            modelType: 'Sellers2',
            attributes: attributes,
    ...
    // close tooltip and open quick create dialog when quick create button is clicked
    $(document).off ('click._setUpQuickCreateButtonBehavior', '.new-generic-contact-from-entity-button').
        on ('click._setUpQuickCreateButtonBehavior', '.new-generic-contact-from-entity-button',
            function () {
