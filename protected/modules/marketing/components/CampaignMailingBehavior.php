...
class CampaignMailingBehavior extends EmailDeliveryBehavior {
    ...
    */
    public static function sendMail($id = null, $t0 = null){
        ...
        $totalSent = 0;
        return;
	$cronEndTime = time() + Yii::app()->settings->batchTimeout;
        try{
            // Get all campaigns that could use mailing
            $campaigns = Campaign::model()->findAllByAttributes(
                    array('complete' => 0, 'active' => 1, 'type' => 'Email'), 'launchdate > 0 AND launchdate < :time AND cronJobEnd < :time', array(':time' => time()));
            foreach($campaigns as $campaign){
                //set the time the max length cron job would run 
                $campaign->cronJobEnd = $cronEndTime;
                $campaign->save();

                //here we will check to see if the user's employee record
                $employ = Employees::model()->findByAttributes(array('assignedTo' => $campaign->assignedTo));
                if(!empty($employ)){
                    //get office
                    $office = Offices::model()->findByAttributes(array('nameId' => $employ->c_office_location__c));
                    if($employ->c_exclude_from_listings_mailing__c == 1 || $employ->c_state__c == 'California_5'){
                        $campaign->active = 0;
                        $campaign->save();
                        continue;
                    }
                    //check the office 
                    if(!empty($office) && $office->c_state__c == "California"){
                        $campaign->active = 0;
                        $campaign->save();
                        continue;
                    }
