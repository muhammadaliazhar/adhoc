...
class OpportunitiesController extends x2base {
        ...
        return array(
            ...
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions'=>array('index','view','create','update','search','addUser','removeUser',
                                    'saveChanges','delete','shareOpportunity','inlineEmail','cloneDeal'),
                'users'=>array('@'),
            ),
    ...
    //function to clone the deal
    public function actionCloneDeal($id){
        $model = $this->loadModel($id);
        $newDeal = new Opportunity;

        foreach ($model->getAttributes() as $attr => $val) {
            if($attr != 'id' && $attr != 'nameId' && $attr != 'name'){
                $newDeal->$attr = $val;
            }else if($attr == 'name'){ 
                $newDeal->$attr = $val . "(1)";
            }
        ...
        $newDeal->save();
        $this->redirect(array('view','id'=>$newDeal->id));
    ...
    public function actionView($id) {
        ...
        if($this->checkPermissions($model,'view')){
            ...
            //check to see if set listing lookup, if so check to update values
            if(isset($model->c_listinglookup__c)){
                $list = listings2::model()->findByAttributes(array('nameId' => $model->c_listinglookup__c));
                if(isset($list)){
                        $model->c_Franchise = $list->c_franchisee_operation__c;
                        $model->c_listing_price__c = $list->c_listing_price__c;
                        $model->c_commission_rate_c = $list->c_commission_rate__c;
                        $model->c_realestate = $list->c_real_estate_included__c;
                        $model->c_lastname = $list->c_Seller;
                        $model->save();
                }
            }
...
<br />".Yii::t('app','Link').": ".CHtml::link($model->name,'http://'.Yii::app()->request->getServerName().$this->createUrl('/opportunities/'.$model->id));
    ...
    public function actionCreate() {
        ...
        if(isset($_POST['Opportunity'])) {
            ...
            $shareArray = array();
            if(isset($_POST['Opportunity']['sharedTo']))
                 $shareArray = $_POST['Opportunity']['sharedTo'];
            $uIDs = array();
            ...
            unset($_POST['Opportunity']['sharedTo']);
    ...
    public function actionUpdate($id) {
	    ...
	    $oldAttributes = $model->attributes;
            ...
            if(!isset($_POST['Opportunity']['sharedTo'])){
                ...
                $shareArray = isset($_POST['Opportunity']['sharedTo']) ? $_POST['Opportunity']['sharedTo'] : array();
                $uIDs = array();
                foreach($shareArray as $uname){
                    $user = User::model()->findByAttributes(array('username' => $uname));
                    if(isset($user))$uIDs[] = $user->id;
