...
class AdminController extends X2Controller {
    ...
    public function actions() {
        return array_merge($this->webUpdaterActions, array(
            ...
            /* x2entstart */
            'codeEditor' => array(
                'class' => 'application.components.codeEditor.CodeEditorAction',
            ),
            'codeBrowser' => array(
                'class' => 'application.components.codeEditor.CodeBrowserAction',
            ),
            /* x2entend */            
            'viewLog' => array(
...
//    }
    ...
    protected function beforeAction($action = null) {
        ...
        if ($imAdmin) {
            ...
            $allowNonAdmin = array(
                "AdminImportModels", "AdminExportModels",
                "AdminImportModelRecords", "AdminExportModelRecords",
                "AdminPrepareModelImport", "AdminPrepareModelExport" 
            );

            if (in_array($action, $allowNonAdmin)) {
                $me = User::getMe();
                $groupToUser = GroupToUser::model()->findByAttributes(array('userId' => $me->id));
                $group = Groups::model()->findByPk($groupToUser->groupId);
                $roleToUser = RoleToUser::model()->findByAttributes(array('userId' => $group->id, 'type' => 'group'));
                $role = Roles::model()->findByPk($roleToUser->roleId);
    ...
    public function accessRules() {
        /* return array(
          ...
          'renameModules', 'manageModules', 'createPage', 'contactUs', 'inquiriesWidget', 'viewChangelog', 'toggleUpdater',
    ...
    private function findIrrelevantAuthChildren($moduleIds) {
        $permissionTypes = array(
            ...
            'MinimumRequirements',
            'PrivateFullAccess',
            'PrivateOwnedFullAccess',
            'PrivateReadOnlyAccess',
            'PrivateOwnedReadOnlyAccess',
            'PrivateUpdateAccess',
            'PrivateOwnedUpdateAccess',
            'ReadOnlyAccess',
            'UpdateAccess',
            'UpdatePrivate',
            'ViewPrivate',
...
//      return;
    ...
    private function createDefaultModulePermissions($moduleName) {
        ...
        $rolePrivateOwnedFullAccess = $auth->createTask($moduleName . 'PrivateOwnedFullAccess');
        ...
        $rolePrivateOwnedUpdateAccess = $auth->createTask($moduleName . 'PrivateOwnedUpdateAccess');
        ...
        $rolePrivateOwnedReadOnlyAccess = $auth->createTask($moduleName . 'PrivateOwnedReadOnlyAccess');
        ...
        // Private Owned Read Only
        $rolePrivateOwnedReadOnlyAccess->addChild($moduleName . 'MinimumRequirements');
        $rolePrivateOwnedReadOnlyAccess->addChild($moduleName . 'ViewPrivate');
        $rolePrivateOwnedReadOnlyAccess->addChild($moduleName . 'QuickView');
        $rolePrivateOwnedReadOnlyAccess->addChild($moduleName . 'InlineEmail');
        $rolePrivateOwnedReadOnlyAccess->addChild($moduleName . 'MobileActionHistoryPublish');
        $rolePrivateOwnedReadOnlyAccess->addChild($moduleName . 'MobileActionHistoryAttachmentsPublish');
        ...
        // Private Owned Update Access
        $rolePrivateOwnedUpdateAccess->addChild($moduleName . 'MinimumRequirements');
        $rolePrivateOwnedUpdateAccess->addChild($moduleName . 'UpdatePrivate');
        $rolePrivateOwnedUpdateAccess->addChild($moduleName . 'GetX2ModelInput');
        ...
        // Private Owned Full Access
        $rolePrivateOwnedFullAccess->addChild($moduleName . 'MinimumRequirements');
        $rolePrivateOwnedFullAccess->addChild($moduleName . 'DeletePrivate');
        ...
        $adminRole->addChild($moduleName . 'PrivateOwnedUpdateAccess');
    ...
    private function deleteTable($moduleName) {
        ...
        $auth->removeAuthItem($ucName . 'PrivateOwnedFullAccess');
        $auth->removeAuthItem($ucName . 'UpdateAccess');
        $auth->removeAuthItem($ucName . 'PrivateUpdateAccess');
        $auth->removeAuthItem($ucName . 'PrivateOwnedUpdateAccess');
        ...
        $auth->removeAuthItem($ucName . 'PrivateOwnedReadOnlyAccess');
    ...
    public function actionExportModelRecords($page, $model) {
        ...
        // If requested, write out CSV header
        if(isset($_SESSION['writeCSVHeader']) && $_SESSION['writeCSVHeader'] === 1) {
            if ($model === 'Contacts') {
                $hdrFields = array('firstName', 'lastName', 'phone', 'c_mobilephone', 'email', 'address', 'address2', 'city', 'state', 'zipcode', 'country', 'assignedTo', 'leadSource', 'createDate', 'c_lead_generated_by__c', 'c_stage__c', 'c_status__c', 'c_listinglookup__c', 'c_ddp_listing_number__c', 'c_docusign_status');
                fputcsv($fp, $hdrFields);
            } else if ($model === 'Listings2') {
                $hdrFields = array('name', 'createDate', 'c_down_payment_requested__c', 'c_listing_price__c', 'c_listing_category__c', 'c_city__c', 'c_county__c', 'c_state__c', 'c_country__c', 'c_employee__c', 'c_franchisee__c', 'c_listing_number__c');
                fputcsv($fp, $hdrFields);
            }
        }
        ...
        foreach ($records as $record) {
            ...
            $combinedMeta = isset($hdrFields) ? array_combine($hdrFields, $hdrFields) : array_combine($_SESSION['modelExportMeta'], $_SESSION['modelExportMeta']);
            ...
            if ($model === 'Contacts') {
                $inqer = X2Model::model('Inquiries')->findByAttributes(array('c_contact__c' =>  $record->nameId, 'c_listing__c' => Listings2::model()->findByPk($_SESSION['exportModelCriteria']->params[':listingId'])->nameId));
                if(isset($inqer)){
                    $tempAttributes['c_docusign_status'] = $inqer->c_docusign_status;
                } else {
                    $tempAttributes['c_docusign_status'] = '';
                }
            }
...
//  $file = Yii::app()->file->set($this->safePath($file));
    ...
    public function actionDownloadData($file) {
        $delete = false;
        if (isset($_GET['deleteAfter'])) $delete = $_GET['deleteAfter'];
        $this->sendFile($file, $delete);
    }
