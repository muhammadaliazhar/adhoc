...
class ProfileController extends x2base {
        ...
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array(
                    ...
                    'setWidgetOrder', 'saveGroupLayout', 'setProfileLayout', 'profiles', 'settings', 'deleteSound', 'deleteBackground',
	...
	$currentUser = CActiveRecord::model('User')->findByPk(Yii::app()->user->getId());
        $currentProfile = CActiveRecord::model('Profile')->findByAttributes(array('username'=>$currentUser->username));
	if ($currentProfile->username == $currentUser->username || Yii::app()->params->isAdmin) {
            $model = $this->loadModel($currentProfile->id);
            $users = User::getNames();
	...
	$profile = Profile::model()->findByAttributes(array('username'=>$userModel->username));
    ...
    public function actionDeleteActivityReport($id, $deleteKey) {
        ...
        if ($deleteKey == $eventData['deleteKey']) {
            $report = X2Model::model('EmailReport')->
	$profileModel = Profile::model()->findByfindByAttributes(array('cronId' => $id));
            $event->delete();
            if (isset($report)) {
                $report->delete();
            }
