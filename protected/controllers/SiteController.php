...
class SiteController extends x2base {
        ...
        return array(
            array('allow',
                'actions' => array(
                    ...
                    'mobileResetPassword', 'webleadCaptcha', 'needsTwoFactor', 'docusign'),
                'users' => array('*'),
            ),
    ...
    public function actionDocusign() {
        ...
        */
        ...
        //$env = DocusignEnvelope::model()->findByAttributes(array('envelopeId' => $envelope_id));
    ...
    private function handleDefaultUpload($model, $name) {
        ...
        if ($note->save()) {
            /* if($_POST['associationType'] == "listings2") {
                $associatedListing = X2Model::model('Listings2')->findByPk($model->associationId);
		$mediaAssociatedWithListing = X2Model::model('Media')->findByPk($model->id);
		if (!empty($mediaAssociatedWithListing)) {
	            $listingImageUrl = $mediaAssociatedWithListing->getPublicUrl();
		    if (empty($associatedListing->c_listing_imagetemp__c)) {
		        $associatedListing->c_listing_imagetemp__c = $listingImageUrl;
			$associatedListing->save();
	            }
                }
            } */
        } else {
                ...
                try {
                    ...
                    if ($createdFile instanceof Google_Service_Drive_DriveFile) {
                        ...
                        if ($model->save()) {
                            ...
                            $doc->name = $model->name;
                            $doc->lastUpdated = $model->lastUpdated;
                            $doc->visibility = 1;
                            $doc->type = '';
                            $doc->text = '';
                            $doc->assignedTo = Yii::app()->user->getName();
                            $doc->mediaId = $model->id;
                    ...
                    switch ($model->associationType) {
                        ...
                        case 'marketing':
                            if (Auxlib::isAjax()) {
                                ...
                                $attachment->campaign = $model->associationId;
                                $attachment->media = $model->id;
                                $attachment->save();
    ...
    public function actionLogin() {
        ...
        if (Yii::app()->user->isInitialized && !Yii::app()->user->isGuest && Yii::app()->params->isAdmin 
                && isset($_GET['loginAsUserViaAdmin'])) {
            $model = new LoginForm;
            $userModel = User::model()->findByPk($_GET['loginAsUserViaAdmin']);
            $model->username=$userModel->username;
            $model->password='password';
            $this->login($model);           
        } else if (Yii::app()->user->isInitialized && !Yii::app()->user->isGuest) {
...
//        $model->quoteAmount = Formatter::parseCurrency($model->quoteAmount,false);
    ...
    public function actionResolveDuplicates() {
        if (isset($_POST['action'], $_POST['ref'], $_POST['modelName']) && Yii::app()->params->isAdmin) {
            $action = $_POST['action'];
            $ref = $_POST['ref'];
