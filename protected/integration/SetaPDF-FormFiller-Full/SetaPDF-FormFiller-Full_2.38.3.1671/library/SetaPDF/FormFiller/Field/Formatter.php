...
class SetaPDF_FormFiller_Field_Formatter
{
    ...
    )
    {
        ...
        return number_format(
                self::makeNumber($field->getValue()) * 100,
                $nDec,
    ...
    )
    {
        ...
        $time = self::makeDate(SetaPDF_Core_Encoding::convertPdfString($field->getValue()), $format);
        if ($time === false) {
            return '';
        }
    ...
    protected static function AFDate_FormatEx(
        ...
        $date = self::makeDate($field->getValue(), $format);
        if ($date === false) {
            return '';
        }
