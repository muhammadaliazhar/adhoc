 ...
 * @version    $Id: Jpeg.php 1767 2022-08-18 08:19:35Z jan.slabon $
 */
...
class SetaPDF_Core_Image_Jpeg extends SetaPDF_Core_Image
{
                    ...
                    case self::MARKER_SOS:
                        ...
                        while (($buffer = $reader->getBuffer()) !== '') {
                            ...
                            if ($markerFound === (strlen($buffer) - 1)) {
                                $start += ($bufferLen / 2);
                            } else if (strpos("\xD0\xD1\xD2\xD3\xD4\xD5\xD6\xD7\x00", $buffer[$markerFound + 1]) !== false) {
                                $start += ($bufferLen / 2);
                            } else {
                                // valid marker found:
    ...
    public function toXObject(SetaPDF_Core_Document $document)
    {
        ...
        if ($this->_inverted && $colorSpace === 'DeviceCMYK') {
