...
class SetaPDF_Core_Canvas_Text extends SetaPDF_Core_Canvas_StandardOperators
{
  ...
  /** Text State methods **/
    ...
    public function setFont($name, $size = 12.)
    {
        ...
        if ($fonts === false || !$fonts->offsetExists($name)) {
        ...
        if ($this->_canvas->getGraphicStateSync() & SetaPDF_Core_Canvas::GS_SYNC_TEXT) {
            if ($font === null) {
                ...
                if ($reference instanceof SetaPDF_Core_Type_Dictionary || $reference instanceof SetaPDF_Core_Type_IndirectObjectInterface) {
