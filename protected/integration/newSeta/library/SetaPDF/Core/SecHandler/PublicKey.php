 ...
 * @version    $Id: PublicKey.php 1782 2022-09-28 12:55:13Z jan.slabon $
 */
    ...
    implements SetaPDF_Core_SecHandler_SecHandlerInterface
{
    ...
    protected function _prepareEnvelopes(array $recipients, $seed)
    {
        ...
        try {
            ...
            foreach ($permissionGroups AS $permissions => $recipients) {
                ...
                /** @var string[] $data */
                $data = preg_split("/(\r\n\r\n|\n\n|\r\r)/", file_get_contents($tmpFileOut), 2);
    ...
    public function auth($recipientCert = null, $recipientKey = null)
    {
        try {
            ...
            $subFilter = SetaPDF_Core_Type_Dictionary_Helper::getValue($dict, 'SubFilter', null, true);

            switch ($subFilter) {
                case 'adbe.pkcs7.s3':
                case 'adbe.pkcs7.s4':
                    $recipients = SetaPDF_Core_Type_Dictionary_Helper::getValue($dict, 'Recipients');
                    $encryptMetadata = true;
                    break;
                case 'adbe.pkcs7.s5':
                    $cf = SetaPDF_Core_Type_Dictionary::ensureType(
                        SetaPDF_Core_Type_Dictionary_Helper::getValue($dict, 'CF')
                    );
                    $defaultCryptFilter = SetaPDF_Core_Type_Dictionary::ensureType(
                        SetaPDF_Core_Type_Dictionary_Helper::getValue($cf, 'DefaultCryptFilter')
                    );
                    $recipients = SetaPDF_Core_Type_Dictionary_Helper::getValue($defaultCryptFilter, 'Recipients');
                    $encryptMetadata = SetaPDF_Core_Type_Dictionary_Helper::getValue($dict, 'EncryptMetadata', true, true);
                    break;
                default:
                    throw new SetaPDF_Core_SecHandler_Exception(
            ...
            foreach ($recipients AS $recipient) {
                ...
                if ($stat) {
                    ...
                    if (!is_array($perm)) {
                        throw new SetaPDF_Core_SecHandler_Exception('Cannot find permissions in envelope data.');
                    }
                    $perm = $perm['perm'];
