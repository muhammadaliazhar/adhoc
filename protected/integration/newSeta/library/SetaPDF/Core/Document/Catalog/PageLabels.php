 ...
 * @version    $Id: PageLabels.php 1715 2022-05-06 10:18:19Z jan.slabon $
 */
...
class SetaPDF_Core_Document_Catalog_PageLabels
{
    ...
    protected function _getTree($create = false)
    {
        if ($this->_tree === null) {
            ...
            $pageLabels = SetaPDF_Core_Type_Dictionary_Helper::getValue($catalog, 'PageLabels');
            if ($pageLabels === null) {
                if ($create === false) {
                    return null;
                }
                $pageLabels = new SetaPDF_Core_Type_Dictionary();
                $catalog->offsetSet('PageLabels', $this->getDocument()->createNewObject($pageLabels));
            }
            
            $this->_tree = new SetaPDF_Core_DataStructure_NumberTree(
                SetaPDF_Core_Type_Dictionary::ensureType($pageLabels), $this->getDocument()
            );
        }
    ...
    protected function _getRanges()
    {
        if (count($this->_ranges) === 0) {
            ...
            foreach ($values AS $start => $data) {
                $this->_ranges[$start] = SetaPDF_Core_Type_Dictionary::ensureType($data['value'])->toPhp();
            }
    ...
    public function addRange($startPage, $style = null, $prefix ='', $firstPageValue = 1, $encoding = 'UTF-8')
    {
        ...
        /** @var SetaPDF_Core_DataStructure_NumberTree $tree */
