...
class X2List extends X2Model {
    ...
    public function rules(){
        ...
        return array(
            ...
            array('assignedTo, type, logicType', 'length', 'max' => 40),
    ...
    public function queryCriteria($useAccessRules = true){
        ...
        if($this->type == 'dynamic' || $this->type == 'campaign'){
            $tagJoinCount = 0;
            ...
            foreach($criteria as $criterion){
                ...
                if($criterion->attribute == 'tags' && $criterion->value){
                    ...
                    switch($criterion->comparison){
                        ...
                        case 'list':
                            $condition = '(TRUE)';
                            foreach(explode(',', $criterion->value) as $con){
                                $condition .=  'AND t.'.$criterion->attribute . ' LIKE \'%' . str_replace("\\" , "\\\\\\\\" , str_replace('"', "",json_encode($con))) . '%\' ';
                            }
                            $search->addCondition('(' . $condition . ')', $logicMode);
                            break;
                        case 'notList':
...
//        );
    ...
    public function statusCount($type){
       ...
       // $lstTbl = X2ListItem::model()->tableName();
        $count = Yii::app()->db->createCommand(
                        'SELECT COUNT(*) FROM  x2_list_items WHERE listId = :listid AND '.$type.' > 0')
