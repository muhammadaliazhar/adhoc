...
class Events extends X2ActiveRecord {
    ...
    public function saveRaw ($profile, $attachmentData, $runValidation=true, $attributes=null) {
            ...
            try {
                ...
                if (!$media->save () && !$associatedMedia->exists) {
                    ...
                    $doc->name = $media->name;
                    $doc->lastUpdated = $media->lastUpdated;
                    $doc->visibility = 1;
                    $doc->type = '';
                    $doc->text = '';
                    $doc->assignedTo = $media->uploadedBy;
                    $doc->mediaId = $media->id;
    ...
    public function save ($runValidation=true, $attributes=null) {
        if ($this->type !== 'feed' && $this->type !== 'comment' && $this->type !== 'media') {
            return;
        }
    ...
    public function getRecipient () {
        ...
        if ($this->user != $recipUser && $this->associationId != 0) {
            if (Yii::app()->user->getId() == $this->associationId) {
                if(!isset( Yii::app()->params->profile))return "You";
                $recipient = 
                    CHtml::link(
                        Yii::t('app', 'You'), 
