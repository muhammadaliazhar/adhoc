...
class Fields extends CActiveRecord {
    ...
    //this variable will be used to test for loops in linked fields
    ...
    public function rules(){
        ...
        return array(
            ...
            array('linkFieldType','length','max'=>250),
            array('linkField','length','max'=>250),
            array('linkFieldName','length','max'=>250),
    ...
    public static function getFieldsOfModelsWithFieldLevelPermissions () {
     ...
     /**
     * this function will check to ensure there is no dependency loop for linked fields IE 
     * contact has a field based on an account and account has a field based on contact
     *   @return true/false, return true if this field will not cause a loop
     */ 
    public function checkForLinkedFieldLoop () {
        
        
        return self::helperLinkedFieldLoop();
        
    }   
    
    //return true if no loop found
    private function helperLinkedFieldLoop(){
        if(self::$in_use){
            return false;
        }
        self::$in_use = true;
        $Links = Fields::model()
                            ->findAllByAttributes(array('modelName' => $this->linkType, 'linkFieldType' => 'linkField'));
        foreach($Links as $link){
            $RecordLinks = Fields::model()
                            ->findAllByAttributes(array('modelName' => $link->linkType, 'linkFieldType' => 'linkField'));
            foreach($RecordLinks as $Rlink){
                if(!$Rlink->helperLinkedFieldLoop()){
                    return false;
                }
            }
        }
