...
class X2Flow extends X2ActiveRecord {
    ...
    public static function trigger($triggerName, $params = array()) {
        ...
        //load the level appropriate Old Attributes
        if (isset($params['model'])) {
            //$params['model']->loadOldAttributesLevel(self::$_triggerDepth);
           //save the old attributes to help deal with depth
            //$params['model']->setOldAttributesLevel((self::$_triggerDepth + 1));
        }
        //check if run on trigger update to check if only last activity has changed
        //make sure the record update trigger 
        if($triggerName == "RecordUpdateTrigger" && isset($params['model'])){
            $someChange = FALSE;
            $attributes = $params['model']->getAttributes();
            foreach ($attributes as $key => $value){
                if($key != "lastActivity" && $key != "lastUpdated"){
                    if(($params['model']->attributeChanged($key))) $someChange = TRUE;
                }

            }
            if(!$someChange)return NULL;
        ...
        foreach ($flows as &$flow) {
            ...
            $triggerLog->save();
            //adding code so that 
            if($flow->triggerType == "PeriodicTrigger"){
                //first check the next time it should run and if we have not reach that point skip it
                if(!empty($flow->nextRunTime) && time() < $flow->nextRunTime) continue;
                //other wise we are fine to move on but we will update the next Run Time and go on to 
                //wait 17 mins to run again
                $flow->nextRunTime = time() + (60 * 17);
                $flow->save();
        ...
        if (isset($params['model'])) {
            //$params['model']->loadOldAttributesLevel(self::$_triggerDepth);
            //$params['model']->clearOldAttributesLevel((self::$_triggerDepth + 1));
            //update the record og
            //if(self::$_triggerDepth == 0 && $params['model']->id == 2690)printR($params['model'],1);
            //$params['model']->disableUpdateTrigger();
            //$params['model']->update();
            //$params['model']->enableUpdateTrigger();
     ...
     */
    public static function executeFlow(&$flow, &$params, $actionId = null, $macroRun = false) {
        //if this is a check and put in macro then lower the trigger depth
        if($macroRun)self::$_triggerDepth--;
        if (self::$_triggerDepth > self::MAX_TRIGGER_DEPTH) // ...have we delved too deep?
            return;
