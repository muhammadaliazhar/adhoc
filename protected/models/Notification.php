			...
			case 'escalateCase':
				return Yii::t('app', '{user} escalated a Service Case to you: {record}', array(
					...
					'{record}'=>$record->createLink(),
                        ...
                        case 'ShoutOut':
                                return Yii::t('app', '{user} tagged you in a {post}' , array(
                                        '{user}'=>X2Model::model('Profile')->findByPk($this->modelId)->fullName,
                                        '{post}'=> '<a href="' .  Yii::app()->baseUrl .'/index.php/profile/viewevent?postID=' . $this->value .'">Post</a>',

                                ));

                        case 'ShoutOutCom':
                            return Yii::t('app', '{user} tagged you in a comment on a {post}' , array(
                                        '{user}'=>X2Model::model('Profile')->findByPk($this->modelId)->fullName,
                                        '{post}'=> '<a href="' .  Yii::app()->baseUrl .'/index.php/profile/viewevent?postID=' . $this->value .'">Post</a>',
