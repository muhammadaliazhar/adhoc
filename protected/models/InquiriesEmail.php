...
/**
 * InquiriesEmail class. InquiriesEmail is the data structure for taking in and
 * processing data for outbound email, specifically from the inquiries email widget.
 *
 * It is used by the InquiriesEmailForm widget and site/inquiriesEmail, and is
 ...
 * 		rendering of the inquiries email widget {@link InquiriesEmailForm})
 *
 ...
 * @property array $insertableAttributes (read-only) Attributes for the inquiries
 ...
 */
class InquiriesEmail extends CFormModel {
    // Enclosure comments:

    const SIGNATURETAG = 'Signature'; // for signature
    const TRACKTAG = 'OpenedEmail'; // for the tracking image
    const AHTAG = 'ActionHeader'; // for the inquiries action header
...
//				throw new Exception('InquiriesEmail used on a target model name and primary key that matched no existing record.');
        }
     ...
     * sent to people. Since the "To:" field in the inquiries email form is not
     ...
     * email is addressed to and not the model from whose view the inquiries email
    ...
    public function recordEmailSent($makeEvent = true){
        ...
        file_put_contents( "/tmp/testfile.txt", "ran", FILE_APPEND);
        ...
        if($this->contactFlag){
            foreach($recipientContacts as $email => $contact){
                ...
                if($this->targetModel && get_class ($this->targetModel) === 'Contacts'){
                    $skip = ($this->targetModel->id == $contact->id) || ($contact->email == $this->targetModel->email) ;
                ...
                $action->completeDate = $now;
                $action->complete = 'Yes';
		file_put_contents( "/tmp/testfile.txt", " Los ", FILE_APPEND);
                file_put_contents("/tmp/testfile.txt" ,  $action->subject , FILE_APPEND);
		// These attributes are context-sensitive and subject to change:
