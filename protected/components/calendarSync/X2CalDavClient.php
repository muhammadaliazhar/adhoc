...
class X2CalDavClient extends Client {
    ...
    public function patchOutlook($url, $data, $timezone) {
        ...
        $subject = addslashes($data->actionDescription);
    ...
    public function postOutlook($url, $data, $timezone){
        ...
        // Get the notification date and calculate the minutes from the start of the event that the reminder gets triggered
        $notification = Notification::model()->findByAttributes(array('type' => 'action_reminder', 'modelType' => 'Actions', 'modelId' => $data->id));
        if(isset($notification))
            $reminderMinutes = ($data->dueDate - $notification->createDate) / 60;

        $start = "'start': {'dateTime': " . CJSON::encode($dueDate) .",'timeZone': " . CJSON::encode($timezone) . "},";
        $end = "'end': {'dateTime': " . CJSON::encode($completeDate) .",'timeZone': " . CJSON::encode($timezone) . "},";
        $reminder = isset($reminderMinutes) ? "'reminderMinutesBeforeStart': " . CJSON::encode((int)$reminderMinutes) . "," : "";
        $location = "'location': {'displayName': '" . $data->location . "', 'locationType': 'default', 'uniqueId': '" . $data->location . "', 'uniqueIdType': 'private'},";

        $content = '"Body": {"ContentType": "HTML","Content":"' .$data->actionDescriptionTemp.'"},';
        $body = "{'subject': '" . $subject . "'," .$content. $start . $end . $reminder . $location . "}";
    ...
    public function put($url, $eventPath, $data, $etag = null) {
        ...
        file_put_contents('/tmp/calendar.txt', json_encode($response));
