...
class EventTextFormatter {
    ...
    = array()) {
        ...
        file_put_contents('/tmp/content.txt', $event);
	...
	$recipientGroup = isset($event->groupVisibility) && $event->groupVisibility > 1 ? EventGroup::model()->findByPk($event->groupVisibility) : null;
        if ($recipient) {
            $modifier = ' &raquo; ';
        }
        if (isset($recipientGroup)) {
            $recipient = $recipientGroup->name;
            $modifier = ' &raquo; ';
        }
        ...
        if ($params['media'] == null) {
            ...
            if ($recipient && $profileRecipient) {
                ...
                if (!($event->user == Yii::app()->user->getName() && $recipient->username == Yii::app()->user->getName())){
                    $authorText .= $modifier . isset($recipientLink) ? $recipientLink : '';
                }
            }  
            if(count($eventTexts) == 3) {
