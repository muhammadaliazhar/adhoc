...
class FieldFormatter extends FieldFormatterBase {
    ...
    public function renderAttribute(
        ...
        if(($this->owner instanceof Contacts || $this->owner instanceof Opportunity || $this->owner instanceof Listings2 ) && ($fieldName ==  "sharedTo")){

                 $shareArray = json_decode($this->owner->$fieldName);
                $HtmlReturn = "";
                $firstRun = true;
                if(!isset($shareArray) || !is_array($shareArray)) return "";
                foreach($shareArray as $user){
                        $pesrson = Profile::model()->findByPk($user);
                        if(isset($pesrson)){
                                if($firstRun){
                                        $firstRun = false;
                                }else{
                                ...
                                $HtmlReturn .= CHtml::link(
                                        '<span>'.X2Html::encode($pesrson->fullName).'</span>',
                                         "https://sydney.x2developer.com/index.php/profile/" . $user,
                                        array_merge (array(
                                            'class'=>'contact-name'
                                        ), array())
                                    );
        ...
        //this is for render of the title on x2sign envolopes to get them to have links
        if(($this->owner instanceof X2SignEnvelopes) &&  ($fieldName ==  "relatedRecords")){
           $signLinks = X2SignLinks::model()->findAllByAttributes(array('envelopeId' => $this->owner->id));
           $HtmlReturn = ($this->owner->$fieldName);
            if(!empty($HtmlReturn))return $HtmlReturn;
        ...
        //this will render the date fields for open emails and docs viewed
        if(($this->owner instanceof X2SignEnvelopes) &&  ($fieldName ==  "c_EmailOpenedDates")){
           $signLinks = X2SignLinks::model()->findAllByAttributes(array('envelopeId' => $this->owner->id));
            $HtmlReturn = '';
            foreach($signLinks as $link){
                        if(isset($link->openedDate))
                            $HtmlReturn .=  date("Y-m-d H:i:s", $link->openedDate ) . ", ";
        ...
        //this will render the date fields for open emails and docs viewed
        if(($this->owner instanceof X2SignEnvelopes) &&  ($fieldName ==  "c_DocsViewedDates")){
           $signLinks = X2SignLinks::model()->findAllByAttributes(array('envelopeId' => $this->owner->id));
            $HtmlReturn = '';
            foreach($signLinks as $link){
                        if(isset($link->docViewedDate))
                            $HtmlReturn .=  date("Y-m-d H:i:s", $link->docViewedDate ) . ", ";
        ...
        //this code will render links for the docs on nda config
        if(($this->owner instanceof Ndaconfigure) &&  ($fieldName ==  "c_default_nda")){
            $docs = json_decode($this->owner->c_default_nda);
            $HtmlReturn = '';
            if(is_array($docs)){
               foreach($docs as $doc){
                            $ndaDoc=X2SignDocs::model()->findByPk($doc);

                            if(isset($ndaDoc)){
                                $nextDocUrl = Yii::app()->absoluteBaseUrl."/index.php/docs/view/id/" . $ndaDoc->docId;
                                $HtmlReturn .=  '<a href="' . $nextDocUrl  . '">' . $ndaDoc->name . '</a> ';
    ...
    protected function renderLink ($field, $makeLinks, $textOnly, $encode) {
        ...
        if ($linkedModel === null) {
            ...
            return $makeLinks ? $linkedModel->getLink(array(),$fieldName) : $linkedModel->name;
        }
    }
