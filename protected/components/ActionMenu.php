...
class ActionMenu extends X2Widget {
	...
	public function run() {
        ...
        $total = Yii::app()->db->createCommand ("
            select count(*)
            from x2_actions
            where (  assignedTo = '" . Yii::app()->user->getName() . "')  and (type='' or type is null)
        ")->queryScalar ($params);
        $incomplete = Yii::app()->db->createCommand ("
            select count(*)
            from x2_actions
            where (  assignedTo = '" . Yii::app()->user->getName() . "') and (type='' or type is null) and complete='No'
        ")->queryScalar ($params);
		...
		$overdue = Actions::model()->countByAttributes(
            ...
            '(  assignedTo = "' . Yii::app()->user->getName() . '")  AND  dueDate < '.time().' AND (type="" OR type IS NULL)');
		...
		$complete = Actions::model()->countByAttributes(
            ...
            ' ( assignedTo = "' . Yii::app()->user->getName() . '") AND (type="" OR type IS NULL)'
        );
