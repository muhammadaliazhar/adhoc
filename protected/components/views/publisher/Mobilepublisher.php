...
$echoTabRow = function ($tabs, $rowNum=1) use ($that) {
    ?><ul id='<?php echo 'publisher-tabs-row-'.$rowNum; ?>' 
       style='display: none; width:100%; margin: auto; text-align: center;'>
            <?php 
            // Publisher tabs
            foreach ($tabs as $tab) {
                ?> <li style='text-align: center;  margin: auto; width:25%; display: inline-block; margin-right: 0px !important; height: 2em; background-color:transparent;'> <?php
                $tab->renderTitle ("text-align: center; float: none;");
                ?> </li> <?php
            }
            ?>
        </ul><?php    
