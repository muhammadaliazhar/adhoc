...
class X2AuthManager extends CDbAuthManager {
    ...
    public function checkAccess($itemName, $userId, $params = array()) {
        ...
        if (!isset($this->_access))
            $this->_access = array();
                ...
                //to help with groups with save action
 ...
 //if no access check group stuff as last thing
        if(!$hasAccess && ($hasAccess != 1) && isset($params['X2Model']) && isset($ACTION)){
                $rolesIDs = Roles::getUserRoles($userId);
                $roles = array();
                foreach ($rolesIDs as $roleId) {
                        $roles[] = Roles::model()->findByPk ($roleId);
                }

                //now check if st to owned settings and if so check that model is in group owned
                foreach ($roles as $aRole){

                        $resut = $this->db->createCommand()
                        ->select()
                        ->from('x2_auth_item_child')
                        ->where('parent=:RoleName AND child =:Action', array(':RoleName'=>$aRole->name , ':Action' =>  ucfirst(get_class($params['X2Model'])) . 'PrivateOwned' . $ACTION . 'Access' ))
                        ->queryAll();
                        if(!empty($resut)){
                                ...
                                //if($itemName == "ContactsUpdate")printR($generationalGroupmates,1);
                                foreach($generationalGroupmates as $mate){
                                        if(isset($params['X2Model']->assignedTo) && $params['X2Model']->assignedTo == $mate) $hasAccess = 1;
                                }
                        }
         ...
         //if still no has accesss check the share to field
         if(!$hasAccess && isset($params['X2Model']) && isset($params['X2Model']->sharedTo) && ($params['X2Model'] instanceof Opportunity || $params['X2Model'] instanceof Listings2)  && ((Groups::userHasRole(User::getMe()->id, 'Franchise')))){
                   $shareArray = json_decode($params['X2Model']->sharedTo);
                   if(is_array($shareArray ) &&  (in_array(User::getMe()->id , $shareArray) || in_array( '"' .User::getMe()->id . '"' , $shareArray))){
                             $hasAccess = 1;
                   }
                   ...
                   if (!empty($usersGroupIds)){
                            ...
                            foreach($generationalGroupmates as $uId){
                                    if(is_array($shareArray ) && (in_array($uId , $shareArray) || in_array( '"' . $uId . '"' , $shareArray)))
                                          return true;
                            }
         ...
         //if still no has accesss check the share to field
         if(!$hasAccess && isset($params['X2Model']) && isset($params['X2Model']->sharedTo) && ($params['X2Model'] instanceof Contacts)  && ((Groups::userHasRole(User::getMe()->id, 'Franchise')) || ($params['X2Model']->private != 1))){
                   $shareArray = json_decode($params['X2Model']->sharedTo);
                   if(is_array($shareArray ) &&  (in_array(User::getMe()->id , $shareArray) || in_array( '"' .User::getMe()->id . '"' , $shareArray))){
                             $hasAccess = 1;
                   }
         ...
         //if asking for ability to see ownded records return true since all should be able to see ownded
        if($itemName == "ContactsPrivateOwnedReadOnlyAccess" && (Groups::userHasRole(User::getMe()->id, 'Franchise'))) $hasAccess = 1;
