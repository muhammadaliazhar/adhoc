...
class FormView extends RecordView {
    ...
    public function getLayoutData() {
        if (method_exists($this->model,'isQuick') && Yii::app()->urlManager->parseUrl(Yii::app()->request) !== 'listings2/listings2/update'){
            $this->model->setQuick(true);
	    if($this->model->isQuick()) {
		    $attributes = array(
			'model' => ucfirst($this->modelName),
			'version' => "Short Create",
