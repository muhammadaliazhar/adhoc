...
abstract class X2FlowTrigger extends X2FlowItem {
    ...
    public static $genericConditions = array(
        ...
        'in_group' => 'User In Office',	
    );


        public static $genericDates = array(
...
,
    ...
    public static function getGenericDates() {
        return  self::$genericDates;
    }
    ...
    public static function getGenericCondition($type) {
        switch ($type) {
            ...
            case 'RelativeTime':
                return array(
                    'label' => Yii::t('studio', 'Time Frame'),
                    'type' => 'dropdown',
                   
                    'options' => self::$genericDates,
                );
                
            case 'in_group':
                return array(
                    'label' => Yii::t('studio', 'User In Office'),
                    'type' => 'dropdown',
                    'options' => Groups::getGroupsOptions(),
                    'multiple' => 1,
                );
    ...
    public static function checkCondition($condition, &$params) {
        ...
        switch ($condition['type']) {
            ...
            case 'email_open':
                ...
                return false;
            case 'in_group':
                $attrVal = $model->getAttribute("assignedTo");
                if($attrVal == "Anyone")return false;
                $user = User::model()->findByAttributes(array("username" => $attrVal));
                $groupLinks = Yii::app()->db->createCommand()->select('groupId')->from('x2_group_to_user')->where('userId='.$user->id)->queryColumn();
                //check each group and see if it selected
                foreach($groupLinks as $group){
                    if(in_array($group , $value))return true;
                }
                return false;
