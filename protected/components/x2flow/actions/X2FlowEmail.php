...
class X2FlowEmail extends BaseX2FlowEmail {
    ...
    public function execute(&$params) {
        ...
        if (isset($options['body']['value']) && !empty($options['body']['value'])) {
            $eml->scenario = 'custom';
            //ading this so that we will sub out the data if we are trying to send
            $emailBody = $this->parseOption('body', $params);
            if(strpos($emailBody ,"assignedTo.franchisee") !== false){
                    $employee = Employees::model()->findByAttributes(array('c_user__c' => $params['model']->assignedTo));
                    if(isset($employee)){
                        $franch = Franchisees::model()->findByAttributes(array('nameId'=>$employee->c_franchisee__c));
                        //$franchEmail = $franch->c_franchisee_owner_email__c;
                        //$franchUser =X2Model::model('User')->findByAttributes(array('username' => $franch->assignedTo));
                        $emailBody = str_replace("assignedTo.franchisee", $franch->name, $emailBody);
                    }

            }

            $eml->message = InlineEmail::emptyBody($emailBody);
