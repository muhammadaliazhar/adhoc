...
class History extends X2Widget {
    ...
    $associationId, $associationType, $relationships, $historyType) {
        ...
        $historyCriteria = array(
            ...
            'buyerverify' => ' AND type="buyerverify"',
        );
                ...
                $visibilityCondition = ' AND (
                        t.assignedTo IN (
                            ...
                            (t.assignedTo="' . Yii::app()->user->getName() . '"))';
                                ...
                                if($associationType != "contacts"){
                        $visibilityCondition = ' AND (visibility="1" OR assignedTo="' . Yii::app()->user->getName() . '" OR assignedTo= "Anyone")';
                }else{

                        if(Groups::userHasRole(User::getMe()->id, 'Franchise')){
                                $shareSQL = "";
                        ...
                        //added code to deal with uploads on contacts so if completed by show the doc
                        $visibilityCondition = '  AND ( ' . $shareSQL . ' assignedTo="' . Yii::app()->user->getName() . '" OR completedBy = "' . Yii::app()->user->getName() . '")';
                }
    ...
    public function run() {
        if ($this->filters) {
            ...
            $historyTabs = array(
                ...
                'buyerverify' => Yii::t('app', 'Buyer Verified'),
            );
