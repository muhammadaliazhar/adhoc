...
/**
 * Provides an inquiries form for sending email from a view page.
 *
 ...
 * @property InquiriesEmail $model The form model for email handling and delivery
 * @property array $insertableAttributes Can be manually set to specify insertable attributes for this scenario. Otherwise, {@link InquiriesEmail::getInsertableAtributes()} will be used.
 ...
 */
class InquiriesEmailForm extends X2Widget {

    /**
     * @var string $type
     */
    public $type = 'inquiriesEmail'; 
    ...
    /**
     * @var string|null $action inquiries email form action
     */
    public $action; 
     ...
     * @var string $JSClass
     */
    public $JSClass = 'InquiriesEmailEditorManager'; 

    public function getPackages () {
        if (!isset ($this->_packages)) {
            $this->_packages = array_merge (parent::getPackages (), array (
                'InquiriesEmailEditorManager' => array(
                    'baseUrl' => Yii::app()->request->baseUrl,
                    'js' => array(
                        'js/InquiriesEmailResizable.js',
                        'js/inquiriesEmailForm.js',
                    ),
    ...
    public function init(){
        ...
        // If insertable attributes aren't set, use the inquiries email model's 
        ...
        Yii::app()->clientScript->registerScript('storeOriginalInquiriesEmailMessage', 
            'x2.inquiriesEmailOriginalBody = $("#inquiries-email-message").val();', 
        CClientScript::POS_READY); 
            //'.CJSON::encode($this->model->message).';',CClientScript::POS_READY);

        Yii::app()->clientScript->registerScript('toggleInquiriesEmailForm', 
            ($this->startHidden ? 
            "window.hideInquiriesEmail = true;\n" : 
            "window.hideInquiriesEmail = false;\n"
        ), CClientScript::POS_HEAD);
    ...
    public function run(){
        $this->render('application.components.views.inquiriesEmailForm', array(
