...
class BarWidget extends DataWidget {
	...
	//Justin Changes
        $data = $report->instance->getData($columns, array(), true, CJSON::encode($chart->reportId));
        ...
        //code to fix issue where charts will fail if data is empty
       if(!isset($columns[$groups])){
            $columns[$groups] = array(0=>0);
        }
        if(!isset($columns[$categories])){
            $columns[$categories] = array(0=>0);
        }
        if(!isset($columns[$values])){
            $columns[$values] = array(0=>0);
        }
