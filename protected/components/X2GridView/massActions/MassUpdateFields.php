...
class MassUpdateFields extends MassAction {
    ...
    public function renderDialog ($gridId, $modelName) {
        $editableFieldsFieldInfo = FormLayout::model ()->getEditableFieldsInLayout ($modelName,1);
    ...
    public function execute (array $gvSelection) {
        ...
        //special case for sharedTo
        if(isset($_POST['fields']['sharedTo'])){
            unset($fields);
            $userIds =array();
            foreach($_POST['fields']['sharedTo'] as $use){
                $user = User::model()->findByAttributes(array('username' => $use));
                if(isset($user)){
                    $userIds[] = $user->id;
                }
            }
            $fields = array();
            $fields['sharedTo'] = json_encode($userIds);
        }
