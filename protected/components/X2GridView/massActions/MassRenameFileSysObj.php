...
class MassRenameFileSysObj extends BaseDocsMassAction {
    ...
    public function renderDialog ($gridId, $modelName) {
        $users = User::getNames();
        $user = User::getMe()->username;
        unset($users[$user]);
        $model = new DocFolders;
        if (isset($_COOKIE['doc_folder_id_selected'])) {
            $model = DocFolders::model ()->findByPk ((int)$_COOKIE['doc_folder_id_selected']);
        } 
        unset($_COOKIE['doc_folder_id_selected']);
        echo "
            <div class='mass-action-dialog' 
             id='".$this->getDialogId ($gridId)."' style='display: none;'>
                ...
                </br><span class='new-shared-to'>".CHtml::encode (Yii::t('docs', 'Shared To:')).
                "</span>".
                CHtml::dropDownList('sharedTo', Yii::app()->user->getName(), $users, array('multiple' => 'multiple'))
                ."
            </div>";
    }
    ...
    public function getLabel () {
        if (!isset ($this->_label)) {
            $this->_label = Yii::t('app', 'Edit Folder Attributes');
        }
        return $this->_label;
    }
    ...
    public function execute (array $gvSelection) {
        if ((Yii::app()->controller->modelClass !== 'Docs' ||
            ...
            !isset ($_POST['newName'])) && (Yii::app()->controller->modelClass !== 'DocFolders')) {
        ...
        $obj->name = $newName;
        $user = User::getMe()->username;
        if (!is_array($sharedTo)) {
            $sharedToArray = (array)json_decode($sharedTo,true);
        } else {
            $sharedToArray = $sharedTo;
        }
        if (is_array($sharedToArray)) {
            array_push($sharedToArray,$user);
        } else {
            $sharedToArray = array();
            array_push($sharedToArray,$user);
        }
        $obj->sharedTo = json_encode($sharedToArray);
        $successes = 0;
        if ($obj->save (true, array ('name', 'sharedTo'))) {
