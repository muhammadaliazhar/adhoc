...
class X2ControllerPermissionsBehavior extends ControllerPermissionsBehavior {
    ...
    public function beforeAction($action = null) {
        ...
        //extra check for ID from mobile actions
        $mobileActions = array("mobileUpdate");
        if(in_array($actionId ,$mobileActions)){
           $attsArray = json_decode($_GET['atts'], 1);
           $model = $staticModel->findByPk($attsArray['id']);
            if ($model instanceof X2Model) {
                $params['X2Model'] = $model;
            }
        ...
        if (Yii::app()->params->isAdmin || 
            ...
            if (Yii::app()->controller instanceof StudioController) {
                $me = User::getMe();
                $groupToUser = isset($me) ? GroupToUser::model()->findByAttributes(array('userId' => $me->id)) : null;
                ...
                if (isset($role) && $role->id == 13) {
                   return true;
                }
            }

            if (Yii::app()->controller instanceof ReportsController) {
               return true;
            }

            if (isset($model) && (Groups::userHasRole(User::getMe()->id, 'Franchise'))) {
                $mates = X2PermissionsBehavior::getGenerationalGroupmates();
                if (isset($mates)) {
                    foreach ($mates as $mate) {
                        if (!empty($model->assignedTo) && $mate === $model->assignedTo) {
                            return true;
                        }
                    }
                }
            ...
            $myUser = User::getMe();
            $isAgent = Groups::userHasRole($myUser->id, 'Agent');

            if (isset($model) && ($model instanceof X2SignEnvelopes) && $isAgent && $model->assignedTo = $myUser->username) return true;

            if(isset($model->sharedTo) && ((Groups::userHasRole($myUser->id, 'Franchise')) || (!($model instanceof Contacts) || $model->private != 1))){
                  $shareArray = json_decode($model->sharedTo);
                  if(in_array(User::getMe()->id , $shareArray) || in_array( '"' .User::getMe()->id . '"' , $shareArray))
                           return true;
                  //now check if they have group mates that can see it
                  $usersGroupIds = Groups::model()->getUserGroups(Yii::app()->getSuId());
                  $generationalGroupmates = array();
                  if (!empty($usersGroupIds) && !$isAgent){
                           $generationalGroupmates = Groups::model()->getGenerationGroupmatesIDs($usersGroupIds, array(), true);
                           foreach($generationalGroupmates as $uId){
                                if(in_array($uId , $shareArray) || in_array( '"' . $uId . '"' , $shareArray))
                                        return true;
                           }
                  } else if ($isAgent && (!($model instanceof Contacts))) {
                       if(in_array($myUser->id , $shareArray) || in_array( '"' . $myUser->id . '"' , $shareArray)) {
                           return true;
                       } else $this->owner->denied();

                  }
            }


            if (isset($staticModel) && $staticModel instanceof X2Leads && (empty($model->assignedTo) || is_null($model->assignedTo)))
                return true;

            //special text for contacts denied
            if(isset($staticModel) && $staticModel instanceof Contacts && isset($model->private) &&  ($model->private == 1))   throw new CHttpException(
            403, Yii::t('app','This buyer is exclusively working with (' . $model->assignedTo  . ') Please contact support@tworld.com if you would like to gain access.'));
            //if not private show this message
            if(isset($staticModel) && $staticModel instanceof Contacts)   throw new CHttpException(
            403, Yii::t('app' , 'This buyer is not currently shared with you. Please contact support@tworld.com if you believe this to be in error.'));
            ...
            if ($view) { // Only check edit permissions if they're allowed to view
                $edit = (Yii::app()->params->isAdmin || $moduleAdmin) || 
                    Yii::app()->authManager->checkAccess(
                        ...
                        Yii::app()->getSuID(),
                        array('X2Model' => $model, 'ACTION' => 'Update')
                    );
            }
        ...
        foreach ($array as &$item) {
            if (isset($item['url']) && is_array($item['url'])) {
                ...
                if (!isset($item['visible']) || $item['visible'] == true) {
                    ...
                    $item['visible'] = (isset($role) && ($role->id == 13 || $role->name === 'Agent' || $role->name === 'Franchise')) || Yii::app()->params->isAdmin || Yii::app()->user->checkAccess($action, $params) || is_null($authItem);
                }
            } else {
