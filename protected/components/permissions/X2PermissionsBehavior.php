...
class X2PermissionsBehavior extends ModelPermissionsBehavior {
     ...
     * in question shares a group in common with the assignee(s)) and its 
     * ancestral groupmates based on its parent group will be
    ...
    public function getAccessLevel($uid = null) {
        ...
        }elseif (Yii::app()->authManager->checkAccess($module . 'PrivateOwnedReadOnlyAccess', $uid)) {
            if ($accessLevel < self::QUERY_OWNED)
                $accessLevel = self::QUERY_OWNED;
    ...
    $accessLevel, $tableAlias = 't', $paramsNamespace = 'X2PermissionsBehavior', $showHidden = false) {
        ...
        switch ($accessLevel) {
            ...
            // Continue to case for ancestral group visibility / assignment
            case self::QUERY_OWNED:
                // Made visible among the user(s)' groupmates via "User's Groups"
                // visibility setting:
                $groupmatesRegex = self::getGenerationalGroupmatesRegex($prefix,$assignmentAttr);
		$groupmatesRegex = preg_replace('/\s+OR\s+$/', '', $groupmatesRegex);
                if (!empty($groupmatesRegex)) {
                    $ret[] = array(
                        'condition' => "(" . $groupmatesRegex .")",
                        'operator' => 'OR',
                    );
                }
                ...
                switch ($accessLevel) {
                    ...
                    case self::QUERY_PUBLIC:
                        ...
                        if ($this->owner->getAttribute($this->visibilityAttr) ==
                                ...
                                preg_match(
                                        '/' . $groupmatesRegex . '/', $this->owner->getAttribute($this->assignmentAttr))) {
                    ...
                    case self::QUERY_OWNED:
                        ...
                        if (
                                (bool) $this->assignmentAttr &&
                                (bool) ($groupmatesRegex = self::getGenerationalGroupmatesRegexp()) &&
                                preg_match(
                                        '/' . $groupmatesRegex . '/', $this->owner->getAttribute($this->assignmentAttr))) {
                            $hasViewPermission = true;
                            break;
     ...
     * @param array $userNames
     */
    public static function getUsernameListRegexAssignedTo($usernames,$prefix=null,$assignmentAttr=null) {
        $userAssignedToRegexString = "";
        foreach ($usernames as $username) {
            $userAssignedToRegexString .= " ".$prefix.$assignmentAttr."=\"" .$username."\" ";
            $userAssignedToRegexString .= "OR ";
        }
        return $userAssignedToRegexString;
    }

    public static function isOwnedByMe($assignedTo) {
        ...
        if (!empty($usersGroupIds) && ($generationalGroupmates = Yii::app()->cache->get(User::getMe()->username.'_generational_groupmates')) === false) {
            ...
            Yii::app()->cache->set(User::getMe()->username.'_generational_groupmates',$generationalGroupmates);
        }
        foreach ($generationalGroupmates as $groupmate) {
                if ($assignedTo == $groupmate)
                    return true;
        }
        //return User::getMe()->username == $assignedTo;
        return false;
    }
    ...
    public static function getGenerationalGroupmates() {
        $usersGroupIds = Groups::model()->getUserGroups(Yii::app()->getSuId());
        $generationalGroupmates = array();
        if (!empty($usersGroupIds) && ($generationalGroupmates = Yii::app()->cache->get(User::getMe()->username.'_generational_groupmates')) === false) {
            $generationalGroupmates = Groups::model()->getGenerationGroupmates($usersGroupIds, array(), true);
        ...
        return isset($generationalGroupmates) ? $generationalGroupmates : null;
    }

    public static function getGenerationalGroupmatesRegex($prefix=null,$assignmentAttr=null) {
        $usersGroupIds = Groups::model()->getUserGroups(Yii::app()->getSuId());
        $generationalGroupmates = array();
        if (!empty($usersGroupIds) && ($generationalGroupmates = Yii::app()->cache->get(User::getMe()->username.'_generational_groupmates')) === false) {
            $generationalGroupmates = Groups::model()->getGenerationGroupmates($usersGroupIds, array(), true);
	...
	return empty($generationalGroupmates) ? null : self::getUsernameListRegexAssignedTo($generationalGroupmates,$prefix,$assignmentAttr);
    }

    public static function getGenerationalGroupmatesRegexp() {
        $usersGroupIds = Groups::model()->getUserGroups(Yii::app()->getSuId());
        $generationalGroupmates = array();
        if (!empty($usersGroupIds) && ($generationalGroupmates = Yii::app()->cache->get(User::getMe()->username.'_generational_groupmates')) === false) {
            $generationalGroupmates = Groups::model()->getGenerationGroupmates($usersGroupIds, array(), true);
        ...
        return empty($generationalGroupmates) ? null : self::getUsernameListRegex($generationalGroupmates);
    }
