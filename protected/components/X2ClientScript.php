...
//                            '.\\nTo turn these messages off, set YII_DEBUG to false.');
    ...
    /**
     * Performs all the necessary JavaScript/CSS initializations for Real Estate (Property) parts of the app.
     */
    public function registerREMain(){
        $fullscreen = $this->fullscreen;
        $profile = $this->profile;
        $baseUrl = $this->baseUrl;
        $themeUrl = $this->themeUrl;
       ...
       $this->registerPackages($this->getDefaultPackages());
        ...
        if(!$isGuest){
            ...
            /* x2entstart */
        //Nexmo Integration
            //if(Yii::app()->settings->nexmoIntegration)
            //$this->registerScriptFile($baseUrl.'/js/nexmoInfo.js', CClientScript::POS_BEGIN);
            /* x2entend */
        }
      if(!$isGuest && ($profile->language == 'he' || $profile->language == 'fa'))
            $this->registerCss('rtl-language', 'body{text-align:right;}');
