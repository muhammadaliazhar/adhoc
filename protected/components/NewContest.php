 ...
 */
class NewContest extends X2Widget {
...
//get the values for the grids and make sure to set each to show 25 users
    ...
    $time=time();
    $TopList = Yii::app()->db
            ->createCommand("SELECT id , assignedTo, count(assignedTo) FROM x2_listings2  WHERE  c_client_agreement_date__c > " . $StartMonth . " AND c_client_agreement_date__c < " . $EndMonth . "  AND assignedTo != 'Anyone' Group By assignedTo ORDER BY COUNT(assignedTo) DESC")
            ->queryAll();

    //shrink the array size 
    $TopList = array_slice($TopList,0,25);
   
    $this->render('NewContest',
        array(
