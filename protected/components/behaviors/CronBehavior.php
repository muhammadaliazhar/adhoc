...
class CronBehavior extends CBehavior {
	...
	//was used to get docusign updates, no longer needed, left here for refrence
        //if(!isset(Yii::app()->settings->DocusignLastUpdate) || Yii::app()->settings->DocusignLastUpdate < (time() - 6*60*60)){
        //if(false){      

         //       $admin = Yii::app()->settings;
          //      $admin->DocusignLastUpdate = time();
           //     $admin->save();
         //       $DOCS = Docusign_status::model()->findAll("createDate < " . (time() - 3*24*60*60) . " AND " . "createDate > " . (time() - 4*24*60*60) . " AND c_status != 'completed'" );
		
	//	$CheckUsers = array();
         //       $docusign = DocusignBehavior::createDocusignInstance();
        //        foreach($DOCS as $doc){
	//		$docusign->getUpdateDocs($doc->c_documentId);
	//	}	
	//	foreach($DOCS as $doc){
	//	if(false){	
	//		if(!in_array($doc->c_owner , $CheckUsers)){
	//		file_put_contents("/tmp/JACKTIME.txt", " user - " . $doc->c_owner , FILE_APPEND);
	//		$CheckUsers[] = $doc->c_owner;
	//		
	//		$ListOfDocs =  get_object_vars(json_decode($docusign->getUpdateDocsList($doc->c_owner)));
	//			//file_put_contents("/tmp/JACKTIME.txt", " inside - " . json_encode($ListOfDocs) , FILE_APPEND);
	//			foreach($ListOfDocs['envelopes'] as $ENVencode){
	//				$ENV = get_object_vars($ENVencode);
	//				//file_put_contents("/tmp/JACKTIME.txt", " downside - " . $ENV['envelopeId'] , FILE_APPEND);
	//				$DownDoc = Docusign_status::model()->findByAttributes(array('c_envelopeId' => $ENV['envelopeId']));
	//				if(isset($DownDoc) && $DownDoc->c_status != "completed")
	//					$docusign->getUpdateDocs($DownDoc->c_documentId);
	//			}
         //              
        //        	}
	//	}
	   ...
	   //reusing old docusign time value
	           ...
	           $admin->save();
	            ...
	            $camps = Campaign::model()->findAll("launchDate < " . (time() - 7*24*60*60) . " AND complete = '0' AND id != 27603");
