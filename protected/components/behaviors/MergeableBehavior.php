...
class MergeableBehavior extends CActiveRecordBehavior {
    ...
    public function mergeRelatedRecords(X2Model $model, $logMerge = false) {
        ...
        $ret = $this->owner->mergeDocuSignRecords($model, $logMerge);
        if ($logMerge && !empty($ret)) {
            $mergeData['data']['docuSignRecords'] = $ret;
        }
    ...
    /**
     * Merge associated DocuSign status
     */
    public function mergeDocuSignRecords(X2Model $model, $logMerge = false) {
        ...
        if ($logMerge) {
            $ids = Yii::app()->db->createCommand()
                    ...
                    ->where(
                            'c_recordType = :type AND c_recordId = :id', array(':type' => get_class($model), ':id' => $model->id))
                    ->queryColumn();
            $ret = $ids;
        }
        Docusign_status::model()->updateAll(
                array(
            'c_recordId' => $this->owner->id,
            'associationType' => get_class ($this->owner),
                ), 'c_recordType = :type AND c_recordId = :id', array(':type' => get_class($model), ':id' => $model->id));
        return $ret;
                        ...
                        foreach ($mergeData['data'] as $key => $data) {
                            switch ($key) {
                                ...
                                case 'linkFields':
                                    ...
                                    break;
                                case 'docuSignRecords':
                                    $this->owner->unmergeDocuSignRecords($model->id, $data,X2Model::getAssociationType(get_class($model)) );
                                    break;
    ...
    public function unmergeDocuSignRecords($id, $DocIds, $type) {
        X2Model::model('Actions')->Docusign_status::model()($DocIds, array('c_recordId' => $id, 'c_recordType' => $type ));
    }
