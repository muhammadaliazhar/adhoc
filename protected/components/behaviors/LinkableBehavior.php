...
class LinkableBehavior extends CActiveRecordBehavior {
    ...
    */
        public function getUrlLink($htmlOptions=array (),$attribute=null) {
                if (Yii::app()->params->isMobileApp &&
                    !self::isMobileLinkableRecordType (get_class ($this->owner)))
                {
                ...
                if($this->owner instanceof Listings2 && $attribute=="c_listing_number_lookup"){
                        $listingModel = Listings2::model()->findByPk($this->owner->id);
			if(isset($listingModel))
                        $name = $listingModel->c_listing_number__c;
                } else if($this->owner instanceof Listings2 && $attribute=="c_listinglookup__c"){
                        $listingModel = Listings2::model()->findByPk($this->owner->id);
                        if (isset($listingModel) && $listingModel != null) {
	 ...
	 * @return string a link to the model
	 */
	public function getLink($htmlOptions=array (),$fieldName=null) {
		return $this->getUrlLink ($htmlOptions,$fieldName);
	}
     ...
     * 
    */
     public static function getItems($term, $valueAttr='name', $nameAttr='id', $modelClass=null, $categoryTerms=null, $allOption=false) {
        if (!$modelClass)
        ...
        if (isset($model)) {
            ...
            if (!empty($categoryTerms)) {
                ...
                foreach ($categoryTerms as $categoryTerm) {
                    $sql = "SELECT DISTINCT name FROM x2_subcategories as t WHERE  c_category__c REGEXP '^".$categoryTerm."(_[[:digit:]]+)?$' AND c_currencyisocode = '". $currIso . "'";
                    if ($model->asa ('permissions') && $modelClass !== 'Sellers2') {
                        list ($accessCond, $permissionsParams) = $model->getAccessSQLCondition ();
                        //$sql .= ' AND '.$accessCond;
                        $params = array_merge ($params, $permissionsParams);
                    }

                    $sql .= " ORDER BY c_currencyisocode DESC, $valueAttr ASC ";
                    $command = Yii::app()->db->createCommand($sql);
                    $result = $command->queryAll(true, $params);
                    if (empty($result))
                        $result=array();
                    elseif($allOption)
                        array_unshift($result, array('name'=> 'all'));
                    array_push($resultArray, $result);
                }
                array_push($sendBackArray,$resultArray);
                array_push($sendBackArray,'subcategories');
                echo CJSON::encode($sendBackArray);
            } else if ($tableName==="x2_listings2") {
                $sql = "
                ...
                $command = Yii::app()->db->createCommand($sql);
                $resultNames = $command->queryAll(true, $params);
                $sql = "
                    SELECT $nameAttr as id, c_listing_number__c as value
                    FROM " . $tableName . " as t
                    WHERE c_listing_number__c LIKE :qterm";
                if ($model->asa ('permissions')) {
                    list ($accessCond, $permissionsParams) = $model->getAccessSQLCondition ();
                    $sql .= ' AND '.$accessCond;
                    $params = array_merge ($params, $permissionsParams);
                ...
                $sql .= "ORDER BY $valueAttr ASC";
                $command = Yii::app()->db->createCommand($sql);
                $resultListingNumbers = $command->queryAll(true, $params);
                echo CJSON::encode(array_merge($resultNames,$resultListingNumbers));
            } else { 
                $sql = "
                    SELECT $nameAttr as id, $valueAttr as value
                    FROM " . $tableName . " as t
