...
class ModelConversionBehavior extends CActiveRecordBehavior {
    ...
    public static function getActions () {
        return array (
            ...
            'createNewMappings' =>
                'application.components.recordConversion.X2ModelConversionActionMapping',
            'convert' => 
                'application.components.recordConversion.X2ModelConversionAction',

            'getErrorSummary' =>
                'application.components.recordConversion.X2ModelConversionActionGetErrorSummary',
        );
    }
    ...
    */
     ...
     * @return CModel|false 
     */
    public function convert ($targetClass, $force=false,  $fields=null) {
        ...
        $CapName = ucfirst($targetClass);
        $targetModel = new  $CapName ();
        
        //if (!$force && !$this->checkConversionCompatibility ($targetClass)) { 
        //making it have pop up evrey time 
        if (!$force){ 
            return false;
        }
        ...
        if(isset($fields)){
            ...
            foreach($fieldsArray as $OriginalATT => $TargetATT){
                if(!($TargetATT === "DoNotMap") && !($TargetATT === "DoNotMap")){
                    $targetModel->$TargetATT = $attributes[$OriginalATT];
                }
    ...
    public function getConversionIncompatibilityWarnings ($targetClass) {
        ...
        foreach ($leadsAttrs as $name) {
            ...
            $modelFrom = X2Model::getModelTitle (get_class ($this->owner));
            $modelTo = X2Model::getModelTitle ($targetClass);
            
            $fieldTo = ConvertRecords::model()->findByAttributes(
                    array(
                        'fieldFrom' => $name,
                        'modelFrom' => $modelFrom,
                        'modelTo' => $modelTo
           ...
           // if (isset($fieldTo)) {
            //I think $fieldTo Is not set up so taking it out of this section
            if(false){
                $convertMsg = 'Mapping found. Converting '.$name.' to '.$fieldTo['fieldTo'];
            } else {
                $convertMsg = 'No mapping found for '.$name.' in '.
                        $modelTo.'. Please create new mapping';
                $fieldsToConvert[] = $name;
            ...
            $warnings[] = 
                Yii::t('app', 
                    'A field {fieldName} is in {model} but not in {targetModel}. {convertMsg}.',
                    array (
                        '{fieldName}' => $name,
                        '{model}' => $modelFrom,
                        '{targetModel}' => $modelTo,
        ...
        foreach ($sharedAttrs as $name) {
            ...
            if ($sourceField->type !== $targetModelField->type) {
                if (isset ($fieldMap[$originalName])) {
                    $warnings[] = 
                        Yii::t('app', 
                            'The {model} field {fieldName} (type: {fieldType}) maps'.
                            'to the {targetModel} field {targetField} (type: {targetType})'.
                            'but the fields have different types.',  
                            array (
                                ...
                                '{targetType}' => $targetModelField->type,
                            )
                        );
                } else {
                    $warnings[] = 
                        Yii::t('app', 
                            'The field {fieldName} has different types in {model} '.
                            '(type: {fieldType}) and {targetModel} (type: {targetType}).', 
                            array (
                                '{fieldName}' => $name,
                                '{model}' => X2Model::getModelTitle (get_class ($this->owner)),
                                '{targetModel}' => X2Model::getModelTitle ($targetClass),
        ...
        return array($warnings,$fieldsToConvert);
    }
        ...
        $modules = Yii::app()->db->createCommand()
                    ...
                    ->where(array('or',
                                array('and',
                                 'custom = 1', 
                                 'moduleType <> "link"'),
                                array('in','name',$whitelist)))
                    ->queryAll(); 
        
        foreach ($modules as $module) {
            if(array_key_exists($module['name'], X2Model::$associationModels))
                $module['name'] = X2Model::$associationModels[$module['name']];
            if (substr($module['title'],-1) == 's') { //remove plural
                $module['title'] = substr($module['title'],0,-1);
            }
            if (substr($module['title'],-2) == 'ie') { //if ends in 'ies' -> y
                $module['title'] = substr($module['title'],0,-2) . 'y';
            }
            if ($module['title'] == $sourceModelTitle) continue; //don't add current module
            
            
            $options[$module['name']] = $module['title'];
        }
        
        $html = '<p>';
        $html .= 'Convert ' . $sourceModelTitle . ' to: ' . 
                CHtml::dropDownList(
                    'convertTo', 
                    $targetModelClass, 
                    $options,
                    array('onchange' => 'changeErrorDialogue(this.value)')
                );
        $html .= '</p>';
        $html .= $this->getErrorSummary($targetModelClass,$convertMultiple);
        
        return $html;
    }
    
    public function getErrorSummary($targetModelClass,$convertMultiple=false) {
        $sourceModelClass = get_class ($this->owner);
        $sourceModelTitle = X2Model::getModelTitle ($sourceModelClass, true);
        ...
        $incompatibilityWarnings = $this->owner->getConversionIncompatibilityWarnings ($targetModelClass);
        $html = '';
        if (count($incompatibilityWarnings[0]) > 0) {
            $html .= '<div id="dynamic-contact-conversion">'
                    . '<p>';
            ...
            foreach ($incompatibilityWarnings[0] as $message) {
                $html .= '<li>'.CHtml::encode ($message).'</li>';
            }
            $html .= '</ul>'.
            '<p>';

            if (!empty($incompatibilityWarnings[1])){
                $html .= '<strong>New Mappings:</strong>';
            }
            
            
            $listOfFields = X2Model::model($targetModelClass)->getFields();
            
            $FieldNames = array("DoNotMap" => "Do Not Map");
            foreach($listOfFields as $field) {
                if( (($field->fieldName) != 'id') && (($field->fieldName) != 'nameId')){
                    $FieldNames[$field->fieldName] = $field->attributeLabel;
                }
            }
            foreach ($incompatibilityWarnings[1] as $field) {
                
                $html .= '<p>'.CHTML::encode ($field).': '.CHTML::dropDownList($field,'DoNotMap',$FieldNames, array("id" => $field)).'</p>';
            }
            $html .= CHtml::encode (
    ...
    /* Adds new mappings to x2_convert_records
     * @param dict $fields: key is fieldFrom, value is fieldTo
     *  return: None
     */
    public function createNewMappings ($targetClass=null, $fields=null) {
        $modelFrom = X2Model::getModelTitle (get_class ($this->owner));
        $modelTo = X2Model::getModelTitle ($targetClass);
        
        foreach (json_decode($fields) as $fieldFrom=>$fieldTo) {
            $fieldTo = Fields::getPurifier()->purify($fieldTo);
            $sql = "INSERT INTO x2_convert_records (modelFrom, modelTo, fieldFrom, fieldTo) "
                    . "VALUES (:modelFrom, :modelTo, :fieldFrom, :fieldTo)";
            $parameters = array(':modelFrom' => $modelFrom,
                                ':modelTo' => $modelTo,
                                ':fieldFrom' => $fieldFrom,
                                ':fieldTo' => $fieldTo);
