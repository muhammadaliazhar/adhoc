...
class PlancakeEmailParser {
    ...
    public function getBody($returnType=self::PLAINTEXT)
    {
        ...
        if($charset != 'UTF-8' && $charset != 'ASCII' && $charset != '3DUS-ASCII&quot;=' && $charset != '3DUS-ASCII"=' && $charset != '3DUTF-8"' && $charset != '3DUTF-8">' && $charset != '3DUTF-8') { // 3DUS-ASCII is a subset of UTF-8
            // FORMAT=FLOWED, despite being popular in emails, it is not
            // supported by iconv
            $charset = str_replace("FORMAT=FLOWED", "", $charset);
            file_put_contents("/var/www/transworld_crm/charset.txt", $charset);
            //$body = iconv($charset, 'UTF-8//TRANSLIT', $body);
            $body = mb_convert_encoding($body, 'UTF-8', $charset);
            file_put_contents("/var/www/transworld_crm/charsetBody.txt", $body);
            
            if ($body === FALSE) { // iconv/mb_convert_encoding returns FALSE on failure
